...
def W(A,B):return next(A for A in A if B(A))
def P(A):return min(set(A),key=A.count)
def p(I):A=True;I=tuple(map(tuple,I));B=M(I,A,A,A);C=K(B);D=PP(U,C);E=P(D);F=Q(U,E);H=W(B,F);J=G(I,8,1);L=PZ(J,2,H);return[*map(list,L)]