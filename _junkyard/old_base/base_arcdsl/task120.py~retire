...
def PZ(A):F,G=X(A)+1,M(A)+1;H,I=Y(A)-1,L(A)-1;B,C=min(F,H),min(G,I);D,E=max(F,H),max(G,I);J={(A,C)for A in range(B,D+1)}|{(A,E)for A in range(B,D+1)};K={(B,A)for A in range(C,E+1)}|{(D,A)for A in range(C,E+1)};return frozenset(J|K)
def W(A):
	if len(A)==0:return frozenset({})
...
def p(I):I=tuple(map(tuple,I));A=G(I,True,False,True);B=K(W,PZ);C=R(B,A);D=PJ(I,8,C);return[*map(list,D)]