...
def PZ(A):return R(A),PP(A)
def H(A):return tuple(tuple(A[::-1])for A in zip(*A[::-1]))[::-1]
def PS(A):return tuple(A for A in zip(*A[::-1]))
def W(A):return H(G(PS(A)))
def G(A):return A[:len(A)//2]
def K(A,B):return PE(B,V(A),PZ(A))
def Q(A,B,C,D):
...
def p(I):I=tuple(map(tuple,I));A=Q(I,False,True,True);B=PJ(A);C=K(B,I);D=W(C);E=G(D);return[*map(list,E)]