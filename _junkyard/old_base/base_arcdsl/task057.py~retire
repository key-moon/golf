...
def Q(A,B):return PS(B,V(A),H(A))
def G(a,b):return tuple(A+B for(A,B)in zip(a,b))
def W(A,B,C,D):
...
def p(I):A=True;I=tuple(map(tuple,I));C=W(I,A,A,A);D=PP(C);B=Q(D,I);E=G(B,B);return[*map(list,E)]