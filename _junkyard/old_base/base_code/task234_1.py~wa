def p(g):
    ...
    val_cs={v for r in g for v in r if v}
    val_b={}
    for v in val_cs:
        pts=[(i,j) for i,row in enumerate(g) for j,x in enumerate(row) if x==v]
        I=[p[0] for p in pts];J=[p[1] for p in pts]
        val_b[v]=[min(I),max(I),min(J),max(J)]
    # detect the full rectangle (bbox full) vs the L‐shape
    def full(c):
        y1,y2,x1,x2=val_b[c]
        return all(g[i][j]==c for i in range(y1,y2+1) for j in range(x1,x2+1))
    for v in val_cs:
        if full(v): val_rec=v
    ...
    val_hc=[(i,j) for i in range(n) for j in range(m)
            if g[i][j]==val_L and ((j>0   and g[i][j-1]==val_L)
                                     or (j<m-1 and g[i][j+1]==val_L))]
    val_vc=[(i,j) for i in range(n) for j in range(m)
            if g[i][j]==val_L and ((i>0   and g[i-1][j]==val_L)
                                     or (i<n-1 and g[i+1][j]==val_L))]
