...
def p(I):
    ...
    x4 = val_func_compose(val_func_first, val_func_delta)
    x5 = val_func_rbind(val_func_colorcount, x1)
    x6 = val_func_matcher(x5, 2)
    x7 = val_func_chain(x6, x3, val_func_dval_func_neighbors)
    x8 = val_func_rbind(val_func_sfilter, x7)
    x9 = val_func_chain(val_func_first, x8, val_func_toindices)
    x10 = val_func_fork(val_func_subtract, x4, x9)
    x11 = val_func_fork(val_func_shoot, x4, x10)
    x12 = mval_func_apply(x11, x2)
    x13 = val_func_fill(I, x1, x12)
    x14 = mval_func_apply(val_func_delta, x2)
    O = val_func_fill(x13, 0, x14)
    return [*map(list,O)]
