...
def p(I):
    ...
    x3 = val_func_vmirror(x2)
    x4 = val_func_height(x2)
    x5 = val_func_width(x2)
    x6 = val_func_toivec(x4)
    x7 = val_func_tojvec(x5)
    x8 = val_func_add(x7, (0, 2))
    x9 = val_func_add(x6, (2, 0))
    x10 = val_func_shift(x3, x8)
    x11 = val_func_fill(I, 2, x10)
    x12 = val_func_ofcolor(x11, 2)
    x13 = val_func_hmirror(x12)
    x14 = val_func_shift(x13, x9)
    x15 = val_func_fill(x11, 2, x14)
    x16 = val_func_size(x1)
    x17 = val_func_greater(x16, 4)
    O = val_func_branch(x17, I, x15)
    return [*map(list,O)]
