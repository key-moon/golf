...
def PL(A):F,G=L(A)+1,W(A)+1;H,I=V(A)-1,Y(A)-1;B,C=min(F,H),min(G,I);D,E=max(F,H),max(G,I);J={(A,C)for A in range(B,D+1)}|{(A,E)for A in range(B,D+1)};K={(B,A)for A in range(C,E+1)}|{(D,A)for A in range(C,E+1)};return frozenset(J|K)
def Q(A):
	if len(A)==0:return frozenset({})
...
def PZ(A):return len(A)==len(A[0])if isinstance(A,tuple)else PS(A)*PU(A)==len(A)and PS(A)==PU(A)
def K(A,B,C,D):
...
def p(I):I=tuple(map(tuple,I));A=K(I,True,False,True);B=R(A,PZ);C=P(A,B);D=H(Q,PL);E=PJ(D,C);F=PP(I,2,0);G=PV(F,3,E);return[*map(list,G)]