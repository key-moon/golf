...
def Q(A,B,C):return L(M(A,B),V(B,C))
def X(A,B,C,D):
	...
	for F in T:
		...
		M.add(frozenset(N))
	return frozenset(M)
def Y(A):return next(iter(A))
def p(I):A=True;I=tuple(map(tuple,I));B=X(I,A,A,A);C=Y(B);D=Q(I,C,(1,0));return[*map(list,D)]