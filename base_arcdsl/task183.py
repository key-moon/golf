...
def p(I):
    ...
    x3 = val_func_replace(I, 8, 0)
    x4 = val_func_replace(x3, 1, 0)
    x5 = val_func_compress(x4)
    x6 = val_func_width(x2)
    x7 = val_func_halve(x6)
    x8 = val_func_upscale(x5, x7)
    x9 = val_func_ofcolor(x2, 0)
    O = val_func_fill(x8, 0, x9)
    return [*map(list,O)]
