...
def W(A,B,C):return E(Y(A,B),L(B,C))
def J(A,B):return frozenset((A,B)for B in Z(B))
...
def S(a,b):
	...
	return a[0]-b,a[1]-b
def p(I):I=tuple(map(tuple,I));A=U(I,3);B=U(I,4);C=X(A);D=X(B);E=S(D,C);F=M(E);G=J(3,A);H=W(I,G,F);return[*map(list,H)]