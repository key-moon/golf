...
def p(I):
    ...
    x2 = val_func_colorfilter(x1, 8)
    x3 = val_func_difference(x1, x2)
    x4 = val_func_first(x3)
    x5 = val_func_cover(I, x4)
    x6 = val_func_normalize(x4)
    x7 = val_func_lbind(val_func_shift, x6)
    x8 = val_func_apply(val_func_ulcorner, x2)
    x9 = mval_func_apply(x7, x8)
    O = val_func_paint(x5, x9)
    return [*map(list,O)]
