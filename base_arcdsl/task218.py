...
def Q(A,B):return PU(B,V(A),PP(A))
def R(A):return tuple(tuple(A[::-1])for A in zip(*A[::-1]))[::-1]
...
def G(A):return tuple(B for(C,B)in enumerate(A)if A.index(B)==C)
def p(I):A=False;I=tuple(map(tuple,I));B=W(I,A,A,True);C=PS(B);D=Q(C,I);E=G(D);F=PZ(E);H=G(F);J=R(H);return[*map(list,J)]