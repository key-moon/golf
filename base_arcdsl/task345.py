...
def p(I):
    ...
    x7 = val_func_objects(x5, False, False, True)
    x8 = val_func_sfilter(x7, x6)
    x9 = val_func_difference(x7, x8)
    x10 = val_func_colorfilter(x9, 2)
    x11 = mval_func_apply(val_func_toindices, x10)
    x12 = val_func_apply(val_func_urcorner, x8)
    x13 = val_func_shift(x12, (1, 1))
    x14 = val_func_rbind(val_func_shoot, (-1, 0))
    x15 = mval_func_apply(x14, x13)
    x16 = val_func_fill(x5, 2, x15)
    x17 = mval_func_apply(val_func_vfrontier, x11)
    O = val_func_fill(x16, 2, x17)
    return [*map(list,O)]
