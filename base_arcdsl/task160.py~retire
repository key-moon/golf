...
def M(A,B):return B.union(frozenset({A}))
def W(A):return type(A)(B for A in A for B in A)
def p(I):I=tuple(map(tuple,I));B=Y(I,True,False,True);A=P(B,8);C=Z((1,1));D=M((1,1),C);E=W(A);F=R(I,E);H=G(L,A);J=Q(K,D);N=V(J,H);O=PP(F,2,N);return[*map(list,O)]