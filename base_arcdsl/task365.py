...
def p(I):
    I=tuple(map(tuple,I))
    x1 = val_func_width(I)
    x2 = val_func_astuple(9, x1)
    x3 = val_func_canvas(0, x2)
    x4 = val_func_vconcat(I, x3)
    x5 = val_func_objects(x4, False, False, True)
    x6 = val_func_rbind(val_func_colorcount, 2)
    x7 = val_func_argmax(x5, x6)
    O = val_func_subgrid(x7, I)
    return [*map(list,O)]
