...
def p(I):
    ...
    x2 = val_func_order(x1, val_func_uppermost)
    x3 = val_func_first(x2)
    x4 = val_func_remove(x3, x2)
    x5 = val_func_normalize(x3)
    x6 = val_func_lbind(val_func_shift, x5)
    x7 = val_func_compose(x6, val_func_ulcorner)
    x8 = val_func_interval(2, -1, -1)
    x9 = val_func_apply(val_func_tojvec, x8)
    x10 = val_func_rbind(val_func_apply, x9)
    x11 = val_func_lbind(val_func_compose, val_func_size)
    x12 = val_func_lbind(val_func_lbind, val_func_intersection)
    x13 = val_func_compose(x11, x12)
    x14 = val_func_lbind(val_func_lbind, val_func_shift)
    x15 = val_func_chain(x10, x14, x7)
    x16 = val_func_fork(val_func_argmax, x15, x13)
    x17 = mval_func_apply(x16, x4)
    O = val_func_underfill(I, 1, x17)
    return [*map(list,O)]
