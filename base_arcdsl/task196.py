...
def PW(A):
	if len(A)==0:return A
	F,G=M(A);H,I=Q(A);B,C=min(F,H),min(G,I);D,E=max(F,H),max(G,I);J={(A,C)for A in range(B,D+1)}|{(A,E)for A in range(B,D+1)};K={(B,A)for A in range(C,E+1)}|{(D,A)for A in range(C,E+1)};return frozenset(J|K)
def PM(A,B,C):
	G,H=len(A),len(A[0]);D=list(list(A)for A in A)
	for(E,F)in J(C):
		if 0<=E<G and 0<=F<H:D[E][F]=B
	return tuple(tuple(A)for A in D)
def PE(A):return PJ(A)==len(A)and PZ(A)==1
def PL(A):return PZ(A)==len(A)and PJ(A)==1
def R(A,B,C,D):
...
def G(x):return x
def p(I):A=True;I=tuple(map(tuple,I));C=R(I,A,A,A);B=PY(J,C);D=PV(PS,PL,PE);E=H(B,D);F=P(B,E);L=PV(K,G,PW);M=PP(F,L);N=PM(I,3,M);return[*map(list,N)]