...
def p(I):
    ...
    x2 = val_func_rbind(val_func_subgrid, I)
    x3 = val_func_chain(val_func_palette, val_func_trim, x2)
    x4 = val_func_lbind(val_func_contained, 3)
    x5 = val_func_compose(x4, x3)
    x6 = val_func_fork(val_func_add, val_func_height, val_func_width)
    x7 = val_func_compose(val_func_decrement, x6)
    x8 = val_func_fork(val_func_equality, val_func_size, x7)
    x9 = val_func_mfilter(x1, x5)
    x10 = val_func_mfilter(x1, x8)
    x11 = val_func_replace(I, 3, 6)
    x12 = val_func_fill(x11, 2, x9)
    O = val_func_fill(x12, 1, x10)
    return [*map(list,O)]
