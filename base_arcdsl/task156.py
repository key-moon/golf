...
def p(I):
    ...
    x2 = val_func_colorfilter(x1, 4)
    x3 = val_func_compose(val_func_backdrop, val_func_inbox)
    x4 = val_func_argmin(x2, val_func_size)
    x5 = val_func_argmax(x2, val_func_size)
    x6 = x3(x4)
    x7 = x3(x5)
    x8 = val_func_fill(I, 1, x6)
    O = val_func_fill(x8, 2, x7)
    return [*map(list,O)]
