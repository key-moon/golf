...
def val_func_leastval_func_color(element):
    values = [v for r in element for v in r] if isinstance(element, tuple) else [v for v, _ in element]
...
def p(I):
    ...
    x3 = val_func_color(x2)
    x4 = val_func_subgrid(x2, I)
    x5 = val_func_leastval_func_color(x4)
    x6 = val_func_replace(x4, x5, 0)
    x7 = val_func_replace(x6, x3, x5)
    x8 = val_func_height(x7)
    x9 = val_func_divide(x8, 3)
    O = val_func_downscale(x7, x9)
    return [*map(list,O)]
