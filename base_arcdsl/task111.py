...
def Y(a,b):return min(abs(A-C)+abs(B-D)for(A,B)in S(a)for(C,D)in S(b))
...
def K(A,B):return PX(B,M(A),PS(A))
def Q(a,b):return Y(a,b)==1
def G(A,B,C,D):
...
def R(A,B):return next(A for A in A if B(A))
def PE(A):return len(A)
def p(I):A=True;I=tuple(map(tuple,I));B=G(I,A,A,A);C=H(PE,1);D=R(B,C);E=PU(Q,D);F=R(B,E);J=K(F,I);return[*map(list,J)]