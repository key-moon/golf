def p(val_g):
    ...
    val_C=max(set(sum(val_g,[]))-{0},key=lambda v:sum(r.count(v)for r in val_g))
    val_V=set();val_Q=[]
    for val_y in range(val_h):
        for val_x in range(val_w):
            if val_g[val_y][val_x]==val_C and (val_y,val_x) not in val_V:
                ...
                while val_S:
                    ...
                    for dy,dx in((1,0),(-1,0),(0,1),(0,-1)):
                        y1,x1=y+dy,x+dx
                        if 0<=y1<val_h and 0<=x1<val_w and val_g[y1][x1]==val_C and (y1,x1) not in val_V:
    ...
    for y in range(val_h):
        for x in range(val_w):
            if val_g[y][x]!=val_C: val_g[y][x]=0
    return val_g
