def p(val_g):
    ...
    val_sr=min(i for i,r in enumerate(val_g) if val_m in r)
    val_er=max(i for i,r in enumerate(val_g) if val_m in r)
    val_sc=min(j for i,r in enumerate(val_g) for j,v in enumerate(r) if v==val_m)
    val_ec=max(j for i,r in enumerate(val_g) for j,v in enumerate(r) if v==val_m)
    val_h=(val_er-val_sr+1)//3;val_w=(val_ec-val_sc+1)//3
    val_o=[[0]*3 for _ in[0]*3]
    for i,r in enumerate(val_g):
