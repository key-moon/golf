            ...
            if not val_seen[val_y][val_x]:
                ...
                for val_i in range(len(val_comp)):
                    ...
                    for dx,dy in((1,0),(-1,0),(0,1),(0,-1)):
                        x1,y1=x0+dx,y0+dy
                        if 0<=x1<val_w and 0<=y1<val_h and not val_seen[y1][x1] and g[y1][x1]==val_c:
                            val_seen[y1][x1]=1; val_comp.append((x1,y1))
                xs=[x for x,y in val_comp]; ys=[y for x,y in val_comp]
                x0,x1,y0,y1=min(xs),max(xs),min(ys),max(ys)
                val_anoms=[(x,y) for y in range(y0,y1+1) for x in range(x0,x1+1) if g[y][x]!=val_c]
                if len(val_anoms)==1:
                    ax,ay=val_anoms[0]; m=g[ay][ax]
                    for fx in(0,1):
                        for fy in(0,1):
                            x2=(x0+x1-ax) if fx else ax
                            y2=(y0+y1-ay) if fy else ay
                            if g[y2][x2]==val_c: g[y2][x2]=m
                    g[ay][ax]=val_c
    return g
