def p(g):
 val_r=[i for i,row in enumerate(g) if 5 in row];val_r0,val_r1=val_r[0],val_r[-1]
 val_c=[j for j in range(len(g[0])) if any(g[i][j]==5 for i in val_r)];val_c0,val_c1=val_c[0],val_c[-1]
 val_h=[[0]*len(g[0]) for _ in g]
 for val_i in range(val_r0,val_r1+1):
  for val_j in range(val_c0,val_c1+1):val_h[val_i][val_j]=5
 for val_i in range(len(g)):
  for val_j,val_x in enumerate(g[val_i]):
