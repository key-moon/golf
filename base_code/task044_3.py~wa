def p(g):
 val_a=[x for r in g for x in r];val_b=max(val_a,key=val_a.count);val_c=[(1,0),(0,1),(-1,0),(0,-1)]
 def val_d(val_x):
  val_e={(i,j)for i in range(len(g))for j in range(len(g[0]))if g[i][j]==val_x};val_f=[]
  while val_e:
 ...
 for val_m in val_d(val_b):
  ...
  val_p=[(i,j)for i in range(val_r0,val_r1+1)for j in range(val_c0,val_c1+1)if g[i][j]==0]
  ...
  for val_t in set(val_a)-{0,val_b}:
   for val_u in val_d(val_t):
    ...
    if sorted((i-val_m0,j-val_n0)for i,j in val_u)==val_sh:
     for i,j in val_u:g[i][j]=0
     for i,j in val_p:g[i][j]=val_t
 return g
