...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_apply(val_func_delta,B);D=val_func_mfilter(C,val_func_square);E=val_func_fill(A,2,D);return[*map(list,E)]