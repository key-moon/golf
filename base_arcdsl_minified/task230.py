...
def p(A):A=tuple(map(tuple,A));C=val_func_objects(A,True,False,True);B=val_func_apply(val_func_outbox,C);D=val_func_apply(val_func_ulcorner,B);E=val_func_apply(val_func_urcorner,B);F=val_func_apply(val_func_llcorner,B);G=val_func_apply(val_func_lrcorner,B);H=val_func_fill(A,1,D);I=val_func_fill(H,2,E);J=val_func_fill(I,3,F);K=val_func_fill(J,4,G);return[*map(list,K)]