...
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_sizefilter(A,1);D=val_func_difference(A,B);C=val_func_first(D);E=val_func_color(C);F=val_func_lbind(val_func_position,C);G=val_func_fork(val_func_shoot,val_func_center,F);H=mval_func_apply(G,B);J=val_func_fill(I,E,H);return[*map(list,J)]