...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_sizefilter(B,1);E=val_func_difference(B,C);D=val_func_first(E);F=val_func_color(D);G=val_func_lbind(val_func_position,D);H=val_func_fork(val_func_shoot,val_func_center,G);I=mval_func_apply(H,C);J=val_func_fill(A,F,I);return[*map(list,J)]