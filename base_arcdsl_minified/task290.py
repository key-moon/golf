...
def p(A):C=False;A=tuple(map(tuple,A));D=val_func_objects(A,C,C,True);E=val_func_first(D);B=val_func_subgrid(E,A);F=val_func_leastcolor(B);G=val_func_mostcolor(B);H=val_func_switch(B,F,G);return[*map(list,H)]