...
def p(I):B=False;I=tuple(map(tuple,I));C=val_func_objects(I,B,B,True);D=val_func_first(C);A=val_func_subgrid(D,I);E=val_func_leastcolor(A);F=val_func_mostcolor(A);G=val_func_switch(A,E,F);return[*map(list,G)]