...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);D=val_func_colorfilter(B,2);C=val_func_first(D);E=val_func_colorfilter(B,8);F=val_func_first(E);G=val_func_gravitate(C,F);H=val_func_move(A,C,G);return[*map(list,H)]