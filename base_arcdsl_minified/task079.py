...
def p(A):B=True;A=tuple(map(tuple,A));C=val_func_objects(A,B,B,B);D=val_func_totuple(C);E=val_func_apply(val_func_color,D);F=val_func_mostcommon(E);G=val_func_matcher(val_func_color,F);H=val_func_extract(C,G);I=val_func_subgrid(H,A);return[*map(list,I)]