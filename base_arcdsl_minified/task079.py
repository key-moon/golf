...
def p(I):A=True;I=tuple(map(tuple,I));B=val_func_objects(I,A,A,A);C=val_func_totuple(B);D=val_func_apply(val_func_color,C);E=val_func_mostcommon(D);F=val_func_matcher(val_func_color,E);G=val_func_extract(B,F);H=val_func_subgrid(G,I);return[*map(list,H)]