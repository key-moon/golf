...
def val_func_square(A):return len(A)==len(A[0])if isinstance(A,tuple)else val_func_height(A)*val_func_width(A)==len(A)and val_func_height(A)==val_func_width(A)
...
def p(A):D=False;A=tuple(map(tuple,A));E=val_func_objects(A,True,D,D);B=val_func_colorfilter(E,0);C=val_func_sfilter(B,val_func_square);F=val_func_difference(B,C);G=val_func_merge(C);H=val_func_recolor(3,G);I=val_func_merge(F);J=val_func_recolor(4,I);K=val_func_paint(A,H);L=val_func_paint(K,J);return[*map(list,L)]