...
def p(A):A=tuple(map(tuple,A));E=val_func_ofcolor(A,3);B=val_func_replace(A,3,0);F=val_func_dmirror(B);G=pval_func_apply(val_func_pair,B,F);C=val_func_lbind(val_func_apply,val_func_maximum);D=val_func_apply(C,G);H=val_func_cmirror(D);I=pval_func_apply(val_func_pair,D,H);J=val_func_apply(C,I);K=val_func_subgrid(E,J);return[*map(list,K)]