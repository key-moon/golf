...
def p(A):D=False;A=tuple(map(tuple,A));B=val_func_ofcolor(A,4);E=prval_func_apply(val_func_connect,B,B);F=val_func_fork(val_func_either,val_func_vline,val_func_hline);G=val_func_mfilter(E,F);C=underval_func_fill(A,-1,G);H=val_func_objects(C,D,D,True);I=val_func_compose(val_func_backdrop,val_func_inbox);J=mval_func_apply(I,H);K=val_func_fill(C,2,J);L=val_func_replace(K,-1,0);return[*map(list,L)]