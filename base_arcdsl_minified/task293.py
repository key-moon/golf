...
def val_func_objects(grid,univalued,diagonal,without_bg):
	A=grid;H=mostval_func_color(A)if without_bg else None;I=set();D=set();L,M=len(A),len(A[0]);N=val_func_asindices(A);O=val_func_neighbors if diagonal else val_func_dval_func_neighbors
	for B in N:
		if B in D:continue
		E=A[B[0]][B[1]]
		if E==H:continue
...
def p(I):I=tuple(map(tuple,I));B=val_func_objects(I,True,False,True);C=val_func_totuple(B);D=val_func_apply(val_func_color,C);A=val_func_mostcommon(D);E=val_func_ofval_func_color(I,A);F=val_func_backdrop(E);G=val_func_fill(I,A,F);return[*map(list,G)]