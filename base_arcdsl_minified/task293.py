...
def p(A):A=tuple(map(tuple,A));C=val_func_objects(A,True,False,True);D=val_func_totuple(C);E=val_func_apply(val_func_color,D);B=val_func_mostcommon(E);F=val_func_ofval_func_color(A,B);G=val_func_backdrop(F);H=val_func_fill(A,B,G);return[*map(list,H)]