...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_matcher(val_func_size,6);D=val_func_compose(val_func_flip,C);E=val_func_mfilter(B,C);F=val_func_mfilter(B,D);G=val_func_fill(A,2,E);H=val_func_fill(G,1,F);return[*map(list,H)]