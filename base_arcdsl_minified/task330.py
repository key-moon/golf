...
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_matcher(val_func_size,6);C=val_func_compose(val_func_flip,B);D=val_func_mfilter(A,B);E=val_func_mfilter(A,C);F=val_func_fill(I,2,D);G=val_func_fill(F,1,E);return[*map(list,G)]