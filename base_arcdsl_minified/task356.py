...
def p(A):A=tuple(map(tuple,A));B=val_func_ofcolor(A,8);C=val_func_product(B,B);D=val_func_fork(val_func_connect,val_func_first,val_func_last);E=val_func_apply(D,C);F=val_func_fork(val_func_either,val_func_vline,val_func_hline);G=val_func_mfilter(E,F);H=val_func_underfill(A,8,G);return[*map(list,H)]