...
def p(I):I=tuple(map(tuple,I));A=val_func_ofcolor(I,8);B=val_func_product(A,A);C=val_func_fork(val_func_connect,val_func_first,val_func_last);D=val_func_apply(C,B);E=val_func_fork(val_func_either,val_func_vline,val_func_hline);F=val_func_mfilter(D,E);G=val_func_underfill(I,8,F);return[*map(list,G)]