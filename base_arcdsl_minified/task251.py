...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_objects(A,True,B,B);D=val_func_colorfilter(C,0);E=val_func_rbind(val_func_bordering,A);F=val_func_compose(val_func_flip,E);G=val_func_mfilter(D,F);H=val_func_fill(A,1,G);return[*map(list,H)]