...
def p(A):A=tuple(map(tuple,A));D=val_func_ofcolor(A,2);E=val_func_ofcolor(A,8);F=val_func_center(D);G=val_func_center(E);B=val_func_hfrontier(F);C=val_func_vfrontier(G);H=val_func_intersection(B,C);I=val_func_fill(A,2,B);J=val_func_fill(I,8,C);K=val_func_fill(J,4,H);return[*map(list,K)]