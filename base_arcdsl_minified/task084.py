...
def p(A):A=tuple(map(tuple,A));C=val_func_height(A);B=val_func_decrement(C);D=val_func_decrement(B);E=val_func_astuple(D,1);F=val_func_astuple(B,1);G=val_func_shoot(E,(-1,1));H=val_func_shoot(F,(0,1));I=val_func_fill(A,2,G);J=val_func_fill(I,4,H);return[*map(list,J)]