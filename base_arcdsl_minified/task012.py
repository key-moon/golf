...
def p(I):I=tuple(map(tuple,I));A=val_func_leastcolor(I);D=val_func_replace(I,0,A);B=val_func_leastcolor(D);E=val_func_ofcolor(I,B);F=mval_func_apply(val_func_dval_func_neighbors,E);C=val_func_fill(I,B,F);G=val_func_objects(C,False,True,True);H=val_func_fork(val_func_connect,val_func_ulcorner,val_func_lrcorner);J=val_func_fork(val_func_connect,val_func_llcorner,val_func_urcorner);K=val_func_fork(val_func_combine,H,J);L=mval_func_apply(K,G);M=val_func_fill(C,A,L);return[*map(list,M)]