...
def p(A):A=tuple(map(tuple,A));B=val_func_leastcolor(A);E=val_func_replace(A,0,B);C=val_func_leastcolor(E);F=val_func_ofcolor(A,C);G=mval_func_apply(val_func_dval_func_neighbors,F);D=val_func_fill(A,C,G);H=val_func_objects(D,False,True,True);I=val_func_fork(val_func_connect,val_func_ulcorner,val_func_lrcorner);J=val_func_fork(val_func_connect,val_func_llcorner,val_func_urcorner);K=val_func_fork(val_func_combine,I,J);L=mval_func_apply(K,H);M=val_func_fill(D,B,L);return[*map(list,M)]