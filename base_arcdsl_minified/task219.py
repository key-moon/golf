...
def p(I):A=True;I=tuple(map(tuple,I));D=val_func_objects(I,A,A,A);B=val_func_order(D,val_func_uppermost);C=val_func_first(B);E=val_func_remove(C,B);F=val_func_normalize(C);G=val_func_lbind(val_func_shift,F);H=val_func_compose(G,val_func_ulcorner);J=val_func_interval(2,-1,-1);K=val_func_apply(val_func_tojvec,J);L=val_func_rbind(val_func_apply,K);M=val_func_lbind(val_func_compose,val_func_size);N=val_func_lbind(val_func_lbind,val_func_intersection);O=val_func_compose(M,N);P=val_func_lbind(val_func_lbind,val_func_shift);Q=val_func_chain(L,P,H);R=val_func_fork(val_func_argmax,Q,O);S=mval_func_apply(R,E);T=val_func_underfill(I,1,S);return[*map(list,T)]