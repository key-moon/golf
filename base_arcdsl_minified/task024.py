...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_ofval_func_color(A,2);D=mval_func_apply(val_func_vfrontier,C);E=val_func_fill(A,2,D);F=val_func_val_func_colorfilter(B,2);G=val_func_difference(B,F);H=val_func_compose(val_func_hfrontier,val_func_center);I=val_func_fork(reval_func_color,val_func_color,H);J=mval_func_apply(I,G);K=val_func_paint(E,J);return[*map(list,K)]