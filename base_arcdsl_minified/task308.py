...
def p(A):A=tuple(map(tuple,A));D=val_func_mostcolor(A);B=val_func_fgpartition(A);C=val_func_valmax(B,val_func_shape);E=val_func_canvas(D,C);F=val_func_apply(val_func_normalize,B);G=val_func_lbind(val_func_subtract,C);H=val_func_chain(val_func_halve,G,val_func_shape);I=val_func_fork(val_func_shift,val_func_identity,H);J=mval_func_apply(I,F);K=val_func_paint(E,J);return[*map(list,K)]