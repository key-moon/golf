...
def val_func_divide(a,b):
	...
	return a[0]//b,a[1]//b
def p(I):E=False;A=True;I=tuple(map(tuple,I));F=val_func_objects(I,E,A,A);G=val_func_ofcolor(I,4);B=val_func_subgrid(G,I);C=val_func_argmax(F,val_func_lowermost);H=val_func_normalize(C);J=val_func_replace(B,4,0);K=val_func_objects(J,A,E,A);D=val_func_merge(K);L=val_func_width(D);M=val_func_ulcorner(D);N=val_func_width(C);O=val_func_divide(L,N);P=val_func_upscale(H,O);Q=val_func_shift(P,M);R=val_func_paint(B,Q);return[*map(list,R)]