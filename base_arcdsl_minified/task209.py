...
def val_func_divide(A,B):
	...
	return A[0]//B,A[1]//B
def p(A):F=False;B=True;A=tuple(map(tuple,A));G=val_func_objects(A,F,B,B);H=val_func_ofcolor(A,4);C=val_func_subgrid(H,A);D=val_func_argmax(G,val_func_lowermost);I=val_func_normalize(D);J=val_func_replace(C,4,0);K=val_func_objects(J,B,F,B);E=val_func_merge(K);L=val_func_width(E);M=val_func_ulcorner(E);N=val_func_width(D);O=val_func_divide(L,N);P=val_func_upscale(I,O);Q=val_func_shift(P,M);R=val_func_paint(C,Q);return[*map(list,R)]