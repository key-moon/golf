...
def val_func_add(A,B):
	...
	return A[0]+B,A[1]+B
def p(A):A=tuple(map(tuple,A));E=val_func_asindices(A);F=val_func_objects(A,True,False,True);C=val_func_first(F);B=val_func_shift(C,(1,0));G=val_func_fill(A,0,C);H=val_func_paint(G,B);I=val_func_uppermost(B);D=val_func_leftmost(B);J=val_func_subtract(D,10);K=val_func_add(D,10);L=val_func_interval(J,K,2);M=val_func_lbind(val_func_greater,I);N=val_func_compose(M,val_func_first);O=val_func_rbind(val_func_contained,L);P=val_func_compose(O,val_func_last);Q=val_func_sfilter(E,N);R=val_func_sfilter(Q,P);S=val_func_fill(H,4,R);return[*map(list,S)]