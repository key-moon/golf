...
def val_func_add(a,b):
	...
	return a[0]+b,a[1]+b
def p(I):I=tuple(map(tuple,I));D=val_func_asindices(I);E=val_func_objects(I,True,False,True);B=val_func_first(E);A=val_func_shift(B,(1,0));F=val_func_fill(I,0,B);G=val_func_paint(F,A);H=val_func_uppermost(A);C=val_func_leftmost(A);J=val_func_subtract(C,10);K=val_func_add(C,10);L=val_func_interval(J,K,2);M=val_func_lbind(val_func_greater,H);N=val_func_compose(M,val_func_first);O=val_func_rbind(val_func_contained,L);P=val_func_compose(O,val_func_last);Q=val_func_sfilter(D,N);R=val_func_sfilter(Q,P);S=val_func_fill(G,4,R);return[*map(list,S)]