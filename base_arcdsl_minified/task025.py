...
def p(I):I=tuple(map(tuple,I));B=val_func_objects(I,True,False,True);A=val_func_sizefilter(B,1);C=val_func_difference(B,A);D=val_func_apply(val_func_color,C);E=val_func_rbind(val_func_contained,D);F=val_func_compose(E,val_func_color);G=val_func_sfilter(A,F);H=val_func_lbind(val_func_val_func_colorfilter,C);J=val_func_chain(val_func_first,H,val_func_color);K=val_func_fork(val_func_gravitate,val_func_identity,J);L=val_func_fork(val_func_shift,val_func_identity,K);M=mval_func_apply(L,G);N=val_func_merge(A);O=val_func_cover(I,N);P=val_func_paint(O,M);return[*map(list,P)]