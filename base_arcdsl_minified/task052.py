...
def p(A):C=False;A=tuple(map(tuple,A));D=val_func_asindices(A);E=val_func_objects(A,True,C,C);F=val_func_sizefilter(E,3);G=val_func_mfilter(F,val_func_hline);B=val_func_toindices(G);H=val_func_difference(D,B);I=val_func_fill(A,5,B);J=val_func_fill(I,0,H);return[*map(list,J)]