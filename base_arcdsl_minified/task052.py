...
def p(I):B=False;I=tuple(map(tuple,I));C=val_func_asindices(I);D=val_func_objects(I,True,B,B);E=val_func_sizefilter(D,3);F=val_func_mfilter(E,val_func_hline);A=val_func_toindices(F);G=val_func_difference(C,A);H=val_func_fill(I,5,A);J=val_func_fill(H,0,G);return[*map(list,J)]