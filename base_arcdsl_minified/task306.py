...
def p(A):C=False;A=tuple(map(tuple,A));D=val_func_objects(A,True,C,C);B=val_func_colorfilter(D,0);E=val_func_argmin(B,val_func_size);F=val_func_backdrop(E);G=val_func_toobject(F,A);H=val_func_normalize(G);I=val_func_lbind(val_func_shift,H);J=val_func_compose(I,val_func_ulcorner);K=mval_func_apply(J,B);L=val_func_paint(A,K);return[*map(list,L)]