...
def val_func_subtract(A,B):
	...
	return A[0]-B,A[1]-B
def p(A):A=tuple(map(tuple,A));F=val_func_palette(A);G=val_func_objects(A,_A,_A,True);H=val_func_rbind(val_func_greater,1);I=val_func_compose(H,val_func_numcolors);C=val_func_sfilter(G,I);D=val_func_remove(0,F);J=val_func_lbind(val_func_colorcount,A);K=val_func_argmax(D,J);L=val_func_remove(K,D);M=val_func_rbind(val_func_contained,L);N=val_func_compose(M,val_func_first);E=val_func_rbind(val_func_sfilter,N);O=val_func_lbind(val_func_rbind,val_func_subtract);P=val_func_lbind(val_func_occurrences,A);Q=val_func_lbind(val_func_lbind,val_func_shift);R=val_func_compose(O,val_func_ulcorner);S=val_func_chain(R,E,val_func_normalize);T=val_func_chain(P,E,val_func_normalize);U=val_func_fork(val_func_apply,S,T);V=val_func_compose(Q,val_func_normalize);W=val_func_fork(mval_func_apply,V,U);X=val_func_astuple(val_func_cmirror,val_func_dmirror);Y=val_func_astuple(val_func_hmirror,val_func_vmirror);B=val_func_combine(X,Y);Z=val_func_product(B,B);a=val_func_fork(val_func_compose,val_func_first,val_func_last);b=val_func_apply(a,Z);c=val_func_totuple(b);d=val_func_combine(B,c);e=val_func_lbind(val_func_rval_func_apply,d);f=mval_func_apply(e,C);g=mval_func_apply(W,f);h=val_func_paint(A,g);i=val_func_merge(C);j=val_func_cover(h,i);return[*map(list,j)]