...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_sfilter(B,val_func_square);D=val_func_difference(B,C);E=val_func_compose(val_func_backdrop,val_func_inbox);F=mval_func_apply(E,D);G=val_func_replace(A,2,0);H=val_func_fill(G,3,F);return[*map(list,H)]