...
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_sfilter(A,val_func_square);C=val_func_difference(A,B);D=val_func_compose(val_func_backdrop,val_func_inbox);E=mval_func_apply(D,C);F=val_func_replace(I,2,0);G=val_func_fill(F,3,E);return[*map(list,G)]