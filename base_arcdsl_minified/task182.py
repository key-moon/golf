...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);D=val_func_val_func_colorfilter(B,5);E=val_func_fork(val_func_equality,val_func_toindices,val_func_box);F=val_func_extract(D,E);G=val_func_inval_func_box(F);H=val_func_subgrid(G,A);I=val_func_asobject(H);J=val_func_matcher(val_func_first,0);K=val_func_compose(val_func_flip,J);L=val_func_sfilter(I,K);C=val_func_normalize(L);M=val_func_toindices(C);N=val_func_compose(val_func_toindices,val_func_normalize);O=val_func_matcher(N,M);P=val_func_mfilter(B,O);Q=val_func_color(C);R=val_func_fill(A,Q,P);return[*map(list,R)]