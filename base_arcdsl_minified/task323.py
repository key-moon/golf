...
def val_func_subtract(A,B):
	...
	return A[0]-B,A[1]-B
def p(A):A=tuple(map(tuple,A));E=val_func_ofcolor(A,8);F=val_func_center(E);G=val_func_connect((0,0),(1,0));H=val_func_connect((0,0),(0,2));I=val_func_combine(G,H);B=val_func_subtract(F,(2,0));J=val_func_shift(I,B);K=val_func_astuple(-2,2);L=val_func_interval(0,5,1);M=val_func_lbind(val_func_multiply,K);N=val_func_apply(M,L);O=val_func_lbind(val_func_shift,J);C=mval_func_apply(O,N);P=val_func_fill(A,5,C);D=val_func_rot180(P);Q=val_func_ofcolor(D,8);R=val_func_center(Q);S=val_func_subtract(R,B);T=val_func_shift(C,S);U=val_func_toivec(-2);V=val_func_shift(T,U);W=val_func_fill(D,5,V);X=val_func_rot180(W);return[*map(list,X)]