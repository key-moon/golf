...
def p(A):B=True;A=tuple(map(tuple,A));C=val_func_objects(A,B,B,B);D=val_func_order(C,val_func_size);E=val_func_apply(val_func_color,D);F=val_func_rbind(val_func_canvas,(1,1));G=val_func_apply(F,E);H=val_func_merge(G);I=val_func_hmirror(H);return[*map(list,I)]