...
def p(I):I=tuple(map(tuple,I));C=val_func_shape(I);D=val_func_asindices(I);E=val_func_objects(I,True,False,True);A=val_func_argmax(E,val_func_size);F=val_func_mostcolor(A);G=val_func_canvas(0,C);B=val_func_paint(G,A);H=val_func_rbind(val_func_toobject,B);J=val_func_rbind(val_func_colorcount,F);K=val_func_chain(J,H,val_func_neighbors);L=val_func_lbind(val_func_greater,3);M=val_func_compose(L,K);N=val_func_sfilter(D,M);O=val_func_fill(B,0,N);return[*map(list,O)]