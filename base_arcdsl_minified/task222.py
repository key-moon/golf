...
def p(A):A=tuple(map(tuple,A));D=val_func_shape(A);E=val_func_asindices(A);F=val_func_objects(A,True,False,True);B=val_func_argmax(F,val_func_size);G=val_func_mostcolor(B);H=val_func_canvas(0,D);C=val_func_paint(H,B);I=val_func_rbind(val_func_toobject,C);J=val_func_rbind(val_func_colorcount,G);K=val_func_chain(J,I,val_func_neighbors);L=val_func_lbind(val_func_greater,3);M=val_func_compose(L,K);N=val_func_sfilter(E,M);O=val_func_fill(C,0,N);return[*map(list,O)]