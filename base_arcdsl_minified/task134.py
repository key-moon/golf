...
def val_func_divide(A,B):
	...
	return A[0]//B,A[1]//B
def p(A):B=True;A=tuple(map(tuple,A));G=val_func_objects(A,B,B,B);C=val_func_argmax(G,val_func_size);H=val_func_color(C);D=val_func_subgrid(C,A);E=val_func_leastval_func_color(D);I=val_func_replace(D,E,0);F=val_func_replace(I,H,E);J=val_func_height(F);K=val_func_divide(J,3);L=val_func_downscale(F,K);return[*map(list,L)]