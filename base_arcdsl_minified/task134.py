...
def val_func_divide(a,b):
	...
	return a[0]//b,a[1]//b
def p(I):A=True;I=tuple(map(tuple,I));F=val_func_objects(I,A,A,A);B=val_func_argmax(F,val_func_size);G=val_func_color(B);C=val_func_subgrid(B,I);D=val_func_leastval_func_color(C);H=val_func_replace(C,D,0);E=val_func_replace(H,G,D);J=val_func_height(E);K=val_func_divide(J,3);L=val_func_downscale(E,K);return[*map(list,L)]