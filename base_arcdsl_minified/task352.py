...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_colorfilter(B,2);D=mval_func_apply(val_func_outbox,C);E=val_func_fill(A,1,D);return[*map(list,E)]