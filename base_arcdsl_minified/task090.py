...
def p(I):I=tuple(map(tuple,I));A=val_func_interval(2,10,1);B=val_func_prval_func_apply(val_func_astuple,A,A);C=val_func_lbind(val_func_canvas,0);D=val_func_lbind(val_func_occurrences,I);E=val_func_lbind(val_func_lbind,val_func_shift);F=val_func_fork(val_func_apply,E,D);G=val_func_chain(F,val_func_asobject,C);H=mval_func_apply(G,B);J=val_func_argmax(H,val_func_size);K=val_func_fill(I,6,J);return[*map(list,K)]