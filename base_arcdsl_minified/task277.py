...
def p(I):A=True;I=tuple(map(tuple,I));B=val_func_objects(I,A,A,A);C=val_func_totuple(B);D=val_func_apply(val_func_normalize,C);E=val_func_leastcommon(D);F=val_func_matcher(val_func_normalize,E);G=val_func_extract(B,F);H=val_func_replace(I,8,1);J=val_func_fill(H,2,G);return[*map(list,J)]