...
def p(A):B=True;A=tuple(map(tuple,A));C=val_func_objects(A,B,B,B);D=val_func_totuple(C);E=val_func_apply(val_func_normalize,D);F=val_func_leastcommon(E);G=val_func_matcher(val_func_normalize,F);H=val_func_extract(C,G);I=val_func_replace(A,8,1);J=val_func_fill(I,2,H);return[*map(list,J)]