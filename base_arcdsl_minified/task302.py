...
def p(A):C=False;A=tuple(map(tuple,A));D=val_func_objects(A,True,C,C);E=val_func_colorfilter(D,0);B=val_func_sfilter(E,val_func_square);F=val_func_val_func_sizefilter(B,1);G=val_func_merge(F);H=val_func_argmax(B,val_func_size);I=val_func_merge(B);J=val_func_fill(A,7,I);K=val_func_fill(J,8,H);L=val_func_fill(K,6,G);return[*map(list,L)]