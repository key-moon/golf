...
def p(I):B=False;I=tuple(map(tuple,I));C=val_func_objects(I,True,B,B);D=val_func_colorfilter(C,0);A=val_func_sfilter(D,val_func_square);E=val_func_val_func_sizefilter(A,1);F=val_func_merge(E);G=val_func_argmax(A,val_func_size);H=val_func_merge(A);J=val_func_fill(I,7,H);K=val_func_fill(J,8,G);L=val_func_fill(K,6,F);return[*map(list,L)]