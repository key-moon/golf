...
def p(I):I=tuple(map(tuple,I));B=val_func_partition(I);C=val_func_fork(val_func_reval_func_color,val_func_color,val_func_backdrop);A=val_func_apply(C,B);D=val_func_mfilter(A,val_func_hline);E=val_func_mfilter(A,val_func_vline);F=val_func_paint(I,D);G=val_func_paint(F,E);return[*map(list,G)]