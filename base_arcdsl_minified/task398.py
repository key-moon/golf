...
def val_func_multiply(A,B):
	...
	return A[0]*B,A[1]*B
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);D=val_func_size(B);C=val_func_multiply(D,5);E=val_func_astuple(C,C);F=val_func_canvas(0,E);G=val_func_rbind(val_func_shoot,(1,1));H=val_func_compose(G,val_func_center);I=val_func_fork(reval_func_color,val_func_color,H);J=mval_func_apply(I,B);K=val_func_paint(F,J);L=val_func_hmirror(K);return[*map(list,L)]