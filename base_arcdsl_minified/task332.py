...
def p(A):A=tuple(map(tuple,A));B=val_func_vmirror(A);C=val_func_ofcolor(B,5);D=val_func_compose(val_func_even,val_func_last);E=val_func_sfilter(C,D);F=val_func_fill(B,3,E);G=val_func_vmirror(F);return[*map(list,G)]