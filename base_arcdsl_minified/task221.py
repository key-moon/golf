...
def val_func_subtract(a,b):
	...
	return a[0]-b,a[1]-b
def p(I):B=True;I=tuple(map(tuple,I));A=val_func_colorcount(I,0);D=val_func_subtract(9,A);E=val_func_multiply(A,3);F=val_func_multiply(E,A);G=val_func_subtract(F,3);H=val_func_astuple(3,G);J=val_func_canvas(0,H);C=val_func_hconcat(I,J);K=val_func_objects(C,B,B,B);L=val_func_first(K);M=val_func_lbind(val_func_shift,L);N=val_func_compose(M,val_func_tojvec);O=val_func_interval(0,D,1);P=val_func_rbind(val_func_multiply,3);Q=val_func_apply(P,O);R=mval_func_apply(N,Q);S=val_func_paint(C,R);T=val_func_hsplit(S,A);U=val_func_merge(T);return[*map(list,U)]