...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_apply(val_func_color,B);D=val_func_merge(B);E=val_func_lbind(val_func_val_func_colorfilter,B);F=val_func_rbind(val_func_argmax,val_func_rightmost);G=val_func_compose(F,E);H=mval_func_apply(G,C);I=val_func_difference(D,H);J=val_func_move(A,I,(0,1));return[*map(list,J)]