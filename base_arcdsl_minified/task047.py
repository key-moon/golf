...
def p(A):B=True;A=tuple(map(tuple,A));C=val_func_objects(A,B,B,B);E=val_func_fork(val_func_combine,val_func_vfrontier,val_func_hfrontier);D=val_func_compose(E,val_func_center);F=val_func_fork(reval_func_color,val_func_color,D);G=mval_func_apply(F,C);H=val_func_paint(A,G);I=val_func_fork(val_func_intersection,val_func_first,val_func_last);J=val_func_apply(D,C);K=I(J);L=val_func_fill(H,2,K);return[*map(list,L)]