...
def p(I):B=False;I=tuple(map(tuple,I));C=val_func_objects(I,True,B,B);D=val_func_replace(I,5,2);A=val_func_colorfilter(C,5);E=mval_func_apply(val_func_box,A);F=val_func_fill(D,4,E);G=mval_func_apply(val_func_corners,A);H=val_func_fill(F,1,G);return[*map(list,H)]