...
def p(A):C=False;A=tuple(map(tuple,A));D=val_func_objects(A,True,C,C);E=val_func_replace(A,5,2);B=val_func_colorfilter(D,5);F=mval_func_apply(val_func_box,B);G=val_func_fill(E,4,F);H=mval_func_apply(val_func_corners,B);I=val_func_fill(G,1,H);return[*map(list,I)]