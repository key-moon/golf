...
def p(A):A=tuple(map(tuple,A));C=val_func_width(A);D=val_func_height(A);E=val_func_halve(D);F=val_func_astuple(E,C);B=val_func_crop(A,(0,0),F);G=val_func_hmirror(B);H=val_func_vconcat(B,G);return[*map(list,H)]