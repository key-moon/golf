...
def p(I):I=tuple(map(tuple,I));B=val_func_objects(I,True,False,True);C=val_func_lbind(val_func_sizefilter,B);A=val_func_compose(val_func_merge,C);D=A(2);E=A(3);F=A(4);G=val_func_fill(I,3,D);H=val_func_fill(G,2,E);J=val_func_fill(H,1,F);return[*map(list,J)]