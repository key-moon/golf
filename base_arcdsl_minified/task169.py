...
def p(A):A=tuple(map(tuple,A));C=val_func_objects(A,True,False,True);D=val_func_lbind(val_func_sizefilter,C);B=val_func_compose(val_func_merge,D);E=B(2);F=B(3);G=B(4);H=val_func_fill(A,3,E);I=val_func_fill(H,2,F);J=val_func_fill(I,1,G);return[*map(list,J)]