...
def p(I):A=False;I=tuple(map(tuple,I));B=val_func_objects(I,True,A,A);C=val_func_colorfilter(B,1);D=val_func_compose(val_func_size,val_func_delta);E=val_func_argmax(C,D);F=val_func_subgrid(E,I);return[*map(list,F)]