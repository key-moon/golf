...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_objects(A,True,B,B);D=val_func_colorfilter(C,1);E=val_func_compose(val_func_size,val_func_delta);F=val_func_argmax(D,E);G=val_func_subgrid(F,A);return[*map(list,G)]