...
def inval_func_box(patch):A=patch;F,G=val_func_uppermost(A)+1,val_func_leftmost(A)+1;H,I=val_func_lowermost(A)-1,val_func_rightmost(A)-1;B,C=min(F,H),min(G,I);D,E=max(F,H),max(G,I);J={(A,C)for A in range(B,D+1)}|{(A,E)for A in range(B,D+1)};K={(B,A)for A in range(C,E+1)}|{(D,A)for A in range(C,E+1)};return frozenset(J|K)
...
def p(I):E=False;A=True;I=tuple(map(tuple,I));F=val_func_replace(I,5,0);G=val_func_objects(F,A,E,A);H=val_func_apply(val_func_normalize,G);J=val_func_astuple(9,9);B=val_func_canvas(5,J);C=val_func_asindices(B);D=val_func_box(C);K=val_func_center(C);L=val_func_lbind(val_func_contained,0);M=val_func_rbind(val_func_subtract,K);N=val_func_compose(L,M);O=val_func_chain(outval_func_box,outval_func_box,val_func_initset);P=val_func_corners(C);Q=mval_func_apply(O,P);R=val_func_difference(D,Q);S=inval_func_box(D);T=val_func_sfilter(S,N);U=val_func_combine(R,T);V=val_func_fill(B,1,U);W=val_func_objects(V,A,E,A);X=val_func_apply(val_func_toindices,W);Y=val_func_lbind(val_func_matcher,val_func_normalize);Z=val_func_lbind(val_func_extract,X);a=val_func_chain(val_func_ulcorner,Z,Y);b=val_func_compose(a,val_func_toindices);c=val_func_fork(val_func_shift,val_func_identity,b);d=mval_func_apply(c,H);e=val_func_paint(B,d);return[*map(list,e)]