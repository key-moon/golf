...
def p(I):I=tuple(map(tuple,I));A=val_func_leastcolor(I);B=val_func_ofcolor(I,A);D=val_func_prapply(val_func_connect,B,B);E=val_func_fgpartition(I);C=val_func_merge(E);F=val_func_cover(I,C);G=val_func_fork(val_func_either,val_func_hline,val_func_vline);H=val_func_box(C);J=val_func_rbind(val_func_difference,H);K=val_func_chain(val_func_positive,val_func_size,J);L=val_func_fork(val_func_both,G,K);M=val_func_mfilter(D,L);N=val_func_fill(F,A,M);return[*map(list,N)]