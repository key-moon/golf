...
def p(A):A=tuple(map(tuple,A));C=val_func_objects(A,False,True,True);D=val_func_argmin(C,val_func_numcolors);E=val_func_other(C,D);F=val_func_subgrid(E,A);G=val_func_normalize(D);H=val_func_shift(G,(1,1));B=val_func_toindices(H);I=val_func_normalize(E);J=val_func_lbind(val_func_argmin,I);K=val_func_lbind(val_func_rbind,val_func_manhattan);L=val_func_rbind(val_func_compose,val_func_initset);M=val_func_chain(L,K,val_func_initset);N=val_func_chain(val_func_first,J,M);O=val_func_fork(val_func_astuple,N,val_func_identity);P=val_func_apply(O,B);Q=val_func_paint(F,P);R=val_func_fork(val_func_connect,val_func_ulcorner,val_func_lrcorner);S=R(B);T=val_func_fork(val_func_combine,val_func_identity,val_func_vmirror);U=T(S);V=val_func_intersection(B,U);W=val_func_fill(Q,8,V);return[*map(list,W)]