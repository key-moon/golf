...
def p(I):I=tuple(map(tuple,I));B=val_func_objects(I,False,True,True);C=val_func_argmin(B,val_func_numcolors);D=val_func_other(B,C);E=val_func_subgrid(D,I);F=val_func_normalize(C);G=val_func_shift(F,(1,1));A=val_func_toindices(G);H=val_func_normalize(D);J=val_func_lbind(val_func_argmin,H);K=val_func_lbind(val_func_rbind,val_func_manhattan);L=val_func_rbind(val_func_compose,val_func_initset);M=val_func_chain(L,K,val_func_initset);N=val_func_chain(val_func_first,J,M);O=val_func_fork(val_func_astuple,N,val_func_identity);P=val_func_apply(O,A);Q=val_func_paint(E,P);R=val_func_fork(val_func_connect,val_func_ulcorner,val_func_lrcorner);S=R(A);T=val_func_fork(val_func_combine,val_func_identity,val_func_vmirror);U=T(S);V=val_func_intersection(A,U);W=val_func_fill(Q,8,V);return[*map(list,W)]