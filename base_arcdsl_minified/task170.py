...
def val_func_divide(A,B):
	...
	return A[0]//B,A[1]//B
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,False,True,True);E=val_func_argmax(B,val_func_size);F=val_func_argmin(B,val_func_size);C=val_func_subgrid(E,A);D=val_func_subgrid(F,A);G=val_func_width(C);H=val_func_width(D);I=val_func_divide(G,H);J=val_func_downscale(C,I);K=val_func_ofcolor(J,0);L=val_func_fill(D,0,K);return[*map(list,L)]