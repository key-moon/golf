...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_objects(A,True,B,B);D=val_func_colorfilter(C,2);E=val_func_sizefilter(D,1);F=val_func_merge(E);G=val_func_fill(A,1,F);return[*map(list,G)]