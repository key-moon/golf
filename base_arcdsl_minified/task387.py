...
def p(A):A=tuple(map(tuple,A));C=val_func_objects(A,True,False,True);D=val_func_palette(A);E=val_func_remove(0,D);F=val_func_lbind(val_func_other,E);G=val_func_compose(F,val_func_color);H=val_func_fork(reval_func_color,G,val_func_outval_func_box);I=mval_func_apply(H,C);B=mval_func_apply(val_func_toindices,C);J=val_func_box(B);K=val_func_difference(J,B);L=val_func_lbind(val_func_argmin,B);M=val_func_rbind(val_func_compose,val_func_initset);N=val_func_lbind(val_func_rbind,val_func_manhattan);O=val_func_chain(M,N,val_func_initset);P=val_func_chain(val_func_initset,L,O);Q=val_func_fork(val_func_manhattan,val_func_initset,P);R=val_func_compose(val_func_even,Q);S=val_func_sfilter(K,R);T=val_func_paint(A,I);U=val_func_fill(T,5,S);return[*map(list,U)]