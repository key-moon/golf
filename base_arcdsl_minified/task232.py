...
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_apply(val_func_toindices,A);C=val_func_rbind(val_func_shoot,(0,1));D=val_func_compose(C,val_func_center);E=val_func_fork(val_func_reval_func_color,val_func_color,D);F=mval_func_apply(E,A);G=val_func_paint(I,F);H=val_func_interval(0,5,1);J=val_func_apply(val_func_double,H);K=val_func_apply(val_func_increment,J);L=val_func_apply(val_func_tojvec,K);M=val_func_prval_func_apply(val_func_shift,B,L);N=val_func_merge(M);O=val_func_fill(G,5,N);return[*map(list,O)]