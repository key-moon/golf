...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_apply(val_func_toindices,B);D=val_func_rbind(val_func_shoot,(0,1));E=val_func_compose(D,val_func_center);F=val_func_fork(reval_func_color,val_func_color,E);G=mval_func_apply(F,B);H=val_func_paint(A,G);I=val_func_interval(0,5,1);J=val_func_apply(val_func_double,I);K=val_func_apply(val_func_increment,J);L=val_func_apply(val_func_tojvec,K);M=prval_func_apply(val_func_shift,C,L);N=val_func_merge(M);O=val_func_fill(H,5,N);return[*map(list,O)]