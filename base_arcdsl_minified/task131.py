...
def val_func_multiply(a,b):
	...
	return a[0]*b,a[1]*b
def p(I):I=tuple(map(tuple,I));D=val_func_objects(I,True,False,True);E=val_func_colorfilter(D,3);A=val_func_first(E);B=val_func_ofcolor(I,2);C=val_func_gravitate(A,B);F=val_func_first(C);G=val_func_equality(F,0);H=val_func_branch(G,val_func_width,val_func_height);J=H(A);K=val_func_gravitate(B,A);L=val_func_sign(K);M=val_func_multiply(L,J);N=val_func_crement(M);O=val_func_recolor(8,B);P=val_func_shift(O,N);Q=val_func_paint(I,P);R=val_func_move(Q,A,C);return[*map(list,R)]