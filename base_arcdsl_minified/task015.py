...
def p(A):A=tuple(map(tuple,A));B=val_func_ofcolor(A,1);C=val_func_ofcolor(A,2);D=mval_func_apply(val_func_dneighbors,B);E=mval_func_apply(val_func_ineighbors,C);F=val_func_fill(A,7,D);G=val_func_fill(F,4,E);return[*map(list,G)]