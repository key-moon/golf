...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,False,True,True);C=val_func_lbind(val_func_contained,2);D=val_func_compose(C,val_func_palette);E=val_func_sfilter(B,D);F=val_func_size(E);G=val_func_greater(F,1);H=val_func_branch(G,0,8);I=val_func_canvas(H,(1,1));return[*map(list,I)]