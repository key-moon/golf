...
def p(I):B=False;I=tuple(map(tuple,I));A=val_func_objects(I,True,B,B);C=val_func_colorfilter(A,9);D=val_func_colorfilter(A,1);E=val_func_rbind(val_func_bordering,I);F=val_func_compose(val_func_flip,E);G=val_func_mfilter(C,F);H=val_func_rbind(val_func_adjacent,G);J=val_func_mfilter(D,H);K=val_func_fill(I,8,J);return[*map(list,K)]