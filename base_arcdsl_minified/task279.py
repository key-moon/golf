...
def p(A):C=False;A=tuple(map(tuple,A));B=val_func_objects(A,True,C,C);D=val_func_colorfilter(B,9);E=val_func_colorfilter(B,1);F=val_func_rbind(val_func_bordering,A);G=val_func_compose(val_func_flip,F);H=val_func_mfilter(D,G);I=val_func_rbind(val_func_adjacent,H);J=val_func_mfilter(E,I);K=val_func_fill(A,8,J);return[*map(list,K)]