...
def val_func_subtract(A,B):
	...
	return A[0]-B,A[1]-B
def p(A):A=tuple(map(tuple,A));D=val_func_shape(A);B=val_func_replace(A,9,0);E=val_func_lbind(val_func_apply,val_func_maximum);F=val_func_dmirror(B);G=pval_func_apply(val_func_pair,B,F);C=val_func_apply(E,G);H=val_func_subtract(D,(2,2));I=val_func_crop(C,(2,2),H);J=val_func_vmirror(I);K=val_func_objects(J,True,False,True);L=val_func_merge(K);M=val_func_shift(L,(2,2));N=val_func_paint(C,M);return[*map(list,N)]