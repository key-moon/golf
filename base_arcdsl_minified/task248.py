...
def val_func_subtract(a,b):
	...
	return a[0]-b,a[1]-b
def p(I):A=True;I=tuple(map(tuple,I));H=val_func_shape(I);J=val_func_ofcolor(I,1);K=val_func_first(J);L=val_func_shoot(K,(-1,1));B=val_func_fill(I,1,L);M=val_func_ofcolor(B,1);N=val_func_urcorner(M);O=val_func_shoot(N,(-1,-1));C=val_func_fill(B,1,O);P=val_func_objects(C,A,A,A);Q=val_func_first(P);D=val_func_subgrid(Q,C);R=val_func_shape(D);S=val_func_subtract(R,(1,0));E=val_func_crop(D,(1,0),S);F=val_func_vconcat(E,E);G=val_func_vconcat(F,F);T=val_func_vconcat(G,G);U=val_func_hmirror(T);V=val_func_crop(U,(0,0),H);W=val_func_hmirror(V);return[*map(list,W)]