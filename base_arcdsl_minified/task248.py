...
def val_func_subtract(A,B):
	...
	return A[0]-B,A[1]-B
def p(A):B=True;A=tuple(map(tuple,A));I=val_func_shape(A);J=val_func_ofcolor(A,1);K=val_func_first(J);L=val_func_shoot(K,(-1,1));C=val_func_fill(A,1,L);M=val_func_ofcolor(C,1);N=val_func_urcorner(M);O=val_func_shoot(N,(-1,-1));D=val_func_fill(C,1,O);P=val_func_objects(D,B,B,B);Q=val_func_first(P);E=val_func_subgrid(Q,D);R=val_func_shape(E);S=val_func_subtract(R,(1,0));F=val_func_crop(E,(1,0),S);G=val_func_vconcat(F,F);H=val_func_vconcat(G,G);T=val_func_vconcat(H,H);U=val_func_hmirror(T);V=val_func_crop(U,(0,0),I);W=val_func_hmirror(V);return[*map(list,W)]