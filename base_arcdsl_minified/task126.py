...
def p(I):I=tuple(map(tuple,I));A=val_func_height(I);B=val_func_objects(I,True,False,True);C=val_func_compose(val_func_last,val_func_center);D=val_func_apply(C,B);E=val_func_decrement(A);F=val_func_lbind(val_func_astuple,E);G=val_func_apply(F,D);H=val_func_fill(I,4,G);return[*map(list,H)]