...
def p(A):D=False;A=tuple(map(tuple,A));E=val_func_objects(A,True,D,D);B=val_func_sfilter(E,val_func_square);F=val_func_compose(val_func_even,val_func_height);C=val_func_sfilter(B,F);G=val_func_difference(B,C);H=val_func_merge(C);I=val_func_merge(G);J=val_func_fill(A,2,H);K=val_func_fill(J,7,I);return[*map(list,K)]