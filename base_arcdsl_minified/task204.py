...
def val_func_square(piece):A=piece;return len(A)==len(A[0])if isinstance(A,tuple)else val_func_height(A)*val_func_width(A)==len(A)and val_func_height(A)==val_func_width(A)
...
def p(I):C=False;I=tuple(map(tuple,I));D=val_func_objects(I,True,C,C);A=val_func_sfilter(D,val_func_square);E=val_func_compose(val_func_even,val_func_height);B=val_func_sfilter(A,E);F=val_func_difference(A,B);G=val_func_merge(B);H=val_func_merge(F);J=val_func_fill(I,2,G);K=val_func_fill(J,7,H);return[*map(list,K)]