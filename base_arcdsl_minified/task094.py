...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_fork(val_func_combine,val_func_vfrontier,val_func_hfrontier);D=val_func_compose(C,val_func_center);E=mval_func_apply(D,B);F=val_func_underfill(A,6,E);return[*map(list,F)]