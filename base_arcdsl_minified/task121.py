...
def p(A):A=tuple(map(tuple,A));C=val_func_objects(A,False,True,True);D=val_func_matcher(val_func_numcolors,2);B=val_func_extract(C,D);E=val_func_subgrid(B,A);F=val_func_mostcolor(B);G=val_func_replace(E,8,F);return[*map(list,G)]