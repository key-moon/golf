...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_size(B);D=val_func_double(C);E=val_func_interval(0,D,2);F=val_func_apply(val_func_tojvec,E);G=val_func_astuple(1,9);H=val_func_canvas(0,G);I=val_func_fill(H,1,F);J=val_func_hsplit(I,3);K=val_func_merge(J);return[*map(list,K)]