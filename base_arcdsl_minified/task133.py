...
def val_func_subtract(A,B):
	...
	return A[0]-B,A[1]-B
def p(A):H=False;B=True;A=tuple(map(tuple,A));I=val_func_objects(A,H,B,B);C=val_func_objects(A,B,H,B);J=val_func_lbind(val_func_lbind,val_func_colorcount);D=val_func_fork(val_func_apply,J,val_func_palette);K=val_func_compose(val_func_maximum,D);L=val_func_compose(val_func_minimum,D);M=val_func_fork(val_func_subtract,K,L);E=val_func_argmax(I,M);F=val_func_leastcolor(E);G=val_func_normalize(E);N=val_func_matcher(val_func_first,F);O=val_func_sfilter(G,N);P=val_func_ulcorner(O);Q=val_func_colorfilter(C,F);R=val_func_rbind(val_func_toobject,A);S=val_func_lbind(val_func_remove,0);T=val_func_chain(val_func_first,S,val_func_palette);U=val_func_chain(T,R,val_func_outbox);V=val_func_lbind(val_func_multiply,P);W=val_func_compose(V,val_func_width);X=val_func_fork(val_func_subtract,val_func_ulcorner,W);Y=val_func_lbind(val_func_shift,G);Z=val_func_compose(Y,X);a=val_func_fork(val_func_upscale,Z,val_func_width);b=val_func_fork(val_func_recolor,U,a);c=mval_func_apply(b,Q);d=val_func_paint(A,c);e=val_func_merge(C);f=val_func_paint(d,e);return[*map(list,f)]