...
def p(A):A=tuple(map(tuple,A));C=val_func_objects(A,True,False,True);B=val_func_colorfilter(C,6);D=mval_func_apply(val_func_outbox,B);E=val_func_fill(A,3,D);F=mval_func_apply(val_func_delta,B);G=val_func_fill(E,4,F);return[*map(list,G)]