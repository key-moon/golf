...
def val_func_subtract(A,B):
	...
	return A[0]-B,A[1]-B
def p(A):A=tuple(map(tuple,A));D=val_func_objects(A,_A,_A,True);E=val_func_argmax(D,val_func_numcolors);F=val_func_astuple(2,4);G=val_func_rbind(val_func_contained,F);H=val_func_compose(G,val_func_first);C=val_func_rbind(val_func_sfilter,H);I=val_func_lbind(val_func_rbind,val_func_subtract);J=val_func_lbind(val_func_occurrences,A);K=val_func_lbind(val_func_lbind,val_func_shift);L=val_func_compose(I,val_func_ulcorner);M=val_func_chain(L,C,val_func_normalize);N=val_func_chain(J,C,val_func_normalize);O=val_func_fork(val_func_apply,M,N);P=val_func_compose(K,val_func_normalize);Q=val_func_fork(mval_func_apply,P,O);R=val_func_astuple(val_func_cmirror,val_func_dmirror);S=val_func_astuple(val_func_hmirror,val_func_vmirror);B=val_func_combine(R,S);T=val_func_product(B,B);U=val_func_fork(val_func_compose,val_func_first,val_func_last);V=val_func_apply(U,T);W=val_func_totuple(V);X=val_func_combine(B,W);Y=val_func_rval_func_apply(X,E);Z=mval_func_apply(Q,Y);a=val_func_paint(A,Z);return[*map(list,a)]