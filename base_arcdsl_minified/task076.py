...
def val_func_subtract(a,b):
	...
	return a[0]-b,a[1]-b
def p(I):I=tuple(map(tuple,I));C=val_func_objects(I,_A,_A,True);D=val_func_argmax(C,val_func_numcolors);E=val_func_astuple(2,4);F=val_func_rbind(val_func_contained,E);G=val_func_compose(F,val_func_first);B=val_func_rbind(val_func_sfilter,G);H=val_func_lbind(val_func_rbind,val_func_subtract);J=val_func_lbind(val_func_occurrences,I);K=val_func_lbind(val_func_lbind,val_func_shift);L=val_func_compose(H,val_func_ulcorner);M=val_func_chain(L,B,val_func_normalize);N=val_func_chain(J,B,val_func_normalize);O=val_func_fork(val_func_apply,M,N);P=val_func_compose(K,val_func_normalize);Q=val_func_fork(mval_func_apply,P,O);R=val_func_astuple(val_func_cmirror,val_func_dmirror);S=val_func_astuple(val_func_hmirror,val_func_vmirror);A=val_func_combine(R,S);T=val_func_product(A,A);U=val_func_fork(val_func_compose,val_func_first,val_func_last);V=val_func_apply(U,T);W=val_func_totuple(V);X=val_func_combine(A,W);Y=val_func_rval_func_apply(X,D);Z=mval_func_apply(Q,Y);a=val_func_paint(I,Z);return[*map(list,a)]