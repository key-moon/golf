...
def p(A):C=True;A=tuple(map(tuple,A));E=val_func_objects(A,C,C,C);F=val_func_ofcolor(A,9);B=val_func_colorfilter(E,9);G=val_func_rbind(val_func_shoot,(1,0));H=mval_func_apply(G,F);I=underval_func_fill(A,1,H);D=val_func_compose(val_func_halve,val_func_width);J=val_func_rbind(val_func_greater,1);K=val_func_compose(J,D);L=val_func_matcher(D,3);M=val_func_power(val_func_outbox,2);N=val_func_power(val_func_outbox,3);O=mval_func_apply(val_func_outbox,B);P=val_func_sfilter(B,K);Q=val_func_sfilter(B,L);R=mval_func_apply(M,P);S=mval_func_apply(N,Q);T=val_func_fill(I,3,O);U=val_func_fill(T,3,R);V=val_func_fill(U,3,S);return[*map(list,V)]