...
def p(I):I=tuple(map(tuple,I));B=val_func_objects(I,True,False,True);A=val_func_first(B);C=val_func_ulcorner(A);D=val_func_subgrid(A,I);E=val_func_replace(D,0,2);F=val_func_asobject(E);G=val_func_shift(F,C);H=val_func_paint(I,G);return[*map(list,H)]