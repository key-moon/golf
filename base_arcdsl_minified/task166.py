...
def p(A):A=tuple(map(tuple,A));C=val_func_objects(A,True,False,True);B=val_func_first(C);D=val_func_ulcorner(B);E=val_func_subgrid(B,A);F=val_func_replace(E,0,2);G=val_func_asobject(F);H=val_func_shift(G,D);I=val_func_paint(A,H);return[*map(list,I)]