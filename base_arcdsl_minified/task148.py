...
def val_func_subtract(a,b):
	...
	return a[0]-b,a[1]-b
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_ofcolor(I,8);C=val_func_replace(I,8,4);D=val_func_colorfilter(A,2);E=val_func_argmin(A,val_func_uppermost);F=val_func_apply(val_func_uppermost,D);G=val_func_fork(val_func_subtract,val_func_maximum,val_func_minimum);H=G(F);J=val_func_toivec(H);K=val_func_leftmost(E);L=val_func_equality(K,0);M=val_func_branch(L,(0,-1),(0,1));N=val_func_rbind(val_func_shoot,M);O=mval_func_apply(N,B);P=val_func_underfill(C,8,O);Q=val_func_shift(B,J);R=mval_func_apply(val_func_hfrontier,Q);S=val_func_underfill(P,8,R);return[*map(list,S)]