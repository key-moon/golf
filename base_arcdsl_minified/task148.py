...
def val_func_subtract(A,B):
	...
	return A[0]-B,A[1]-B
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_ofcolor(A,8);D=val_func_replace(A,8,4);E=val_func_colorfilter(B,2);F=val_func_argmin(B,val_func_uppermost);G=val_func_apply(val_func_uppermost,E);H=val_func_fork(val_func_subtract,val_func_maximum,val_func_minimum);I=H(G);J=val_func_toivec(I);K=val_func_leftmost(F);L=val_func_equality(K,0);M=val_func_branch(L,(0,-1),(0,1));N=val_func_rbind(val_func_shoot,M);O=mval_func_apply(N,C);P=val_func_underfill(D,8,O);Q=val_func_shift(C,J);R=mval_func_apply(val_func_hfrontier,Q);S=val_func_underfill(P,8,R);return[*map(list,S)]