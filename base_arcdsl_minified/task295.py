...
def p(A):A=tuple(map(tuple,A));D=val_func_width(A);E=val_func_palette(A);F=val_func_halve(D);B=val_func_vupscale(A,F);G=val_func_rbind(val_func_shoot,(1,1));C=val_func_other(E,0);H=val_func_ofcolor(B,C);I=mval_func_apply(G,H);J=val_func_fill(B,C,I);return[*map(list,J)]