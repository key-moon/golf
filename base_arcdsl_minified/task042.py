...
def p(A):B=True;A=tuple(map(tuple,A));C=val_func_objects(A,B,B,B);D=val_func_rbind(val_func_upscale,2);E=val_func_chain(val_func_invert,val_func_halve,val_func_shape);F=val_func_fork(val_func_combine,val_func_hfrontier,val_func_vfrontier);G=val_func_compose(D,val_func_vmirror);H=val_func_fork(val_func_shift,G,E);I=val_func_compose(val_func_toindices,H);J=val_func_lbind(mval_func_apply,F);K=val_func_compose(J,val_func_toindices);L=val_func_fork(val_func_difference,I,K);M=mval_func_apply(L,C);N=val_func_fill(A,8,M);return[*map(list,N)]