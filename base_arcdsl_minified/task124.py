...
def val_func_add(A,B):
	...
	return A[0]+B,A[1]+B
def p(A):C=True;A=tuple(map(tuple,A));D=val_func_width(A);E=val_func_objects(A,C,C,C);B=val_func_first(E);F=val_func_vperiod(B);G=val_func_toivec(F);H=val_func_interval(0,9,1);I=val_func_lbind(val_func_multiply,G);J=val_func_apply(I,H);K=val_func_lbind(val_func_shift,B);L=mval_func_apply(K,J);M=val_func_astuple(D,D);N=val_func_portrait(B);O=val_func_shape(B);P=val_func_add((1,0),O);Q=val_func_decrement(P);R=val_func_shift(B,Q);S=val_func_branch(N,L,R);T=val_func_canvas(0,M);U=val_func_paint(T,B);V=val_func_paint(U,S);return[*map(list,V)]