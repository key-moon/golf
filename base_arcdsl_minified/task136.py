...
def p(A):A=tuple(map(tuple,A));B=val_func_ofcolor(A,2);C=val_func_ofcolor(A,1);D=val_func_lrcorner(B);E=val_func_ulcorner(C);F=val_func_shoot(D,(1,1));G=val_func_shoot(E,(-1,-1));H=val_func_fill(A,2,F);I=val_func_fill(H,1,G);return[*map(list,I)]