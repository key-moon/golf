...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_fork(val_func_difference,val_func_toindices,val_func_box);D=mval_func_apply(C,B);E=val_func_fill(A,0,D);return[*map(list,E)]