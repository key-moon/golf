...
def val_func_multiply(A,B):
	...
	return A[0]*B,A[1]*B
def p(A):C=True;A=tuple(map(tuple,A));D=val_func_objects(A,C,C,C);B=val_func_argmax(D,val_func_size);E=val_func_remove(B,D);F=val_func_lbind(val_func_shift,B);G=val_func_lbind(mval_func_apply,F);H=val_func_double(10);I=val_func_interval(4,H,4);J=val_func_rbind(val_func_apply,I);K=val_func_lbind(val_func_position,B);L=val_func_lbind(val_func_rbind,val_func_multiply);M=val_func_chain(J,L,K);N=val_func_compose(G,M);O=val_func_fork(reval_func_color,val_func_color,N);P=mval_func_apply(O,E);Q=val_func_paint(A,P);return[*map(list,Q)]