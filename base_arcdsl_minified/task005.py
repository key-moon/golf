...
def val_func_multiply(a,b):
	...
	return a[0]*b,a[1]*b
def p(I):B=True;I=tuple(map(tuple,I));C=val_func_objects(I,B,B,B);A=val_func_argmax(C,val_func_size);D=val_func_remove(A,C);E=val_func_lbind(val_func_shift,A);F=val_func_lbind(mval_func_apply,E);G=val_func_double(10);H=val_func_interval(4,G,4);J=val_func_rbind(val_func_apply,H);K=val_func_lbind(val_func_position,A);L=val_func_lbind(val_func_rbind,val_func_multiply);M=val_func_chain(J,L,K);N=val_func_compose(F,M);O=val_func_fork(val_func_reval_func_color,val_func_color,N);P=mval_func_apply(O,D);Q=val_func_paint(I,P);return[*map(list,Q)]