...
def p(A):B=True;A=tuple(map(tuple,A));C=val_func_objects(A,B,B,B);D=val_func_matcher(val_func_size,1);E=val_func_extract(C,D);F=val_func_lbind(val_func_adjacent,E);G=val_func_extract(C,F);H=val_func_subgrid(G,A);return[*map(list,H)]