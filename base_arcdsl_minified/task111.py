...
def p(I):A=True;I=tuple(map(tuple,I));B=val_func_objects(I,A,A,A);C=val_func_matcher(val_func_size,1);D=val_func_extract(B,C);E=val_func_lbind(val_func_adjacent,D);F=val_func_extract(B,E);G=val_func_subgrid(F,I);return[*map(list,G)]