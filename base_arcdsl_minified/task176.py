...
def p(I):C=False;I=tuple(map(tuple,I));D=val_func_objects(I,True,C,C);E=val_func_colorfilter(D,0);F=val_func_compose(val_func_last,val_func_center);A=val_func_order(E,F);B=val_func_size(A);G=val_func_interval(0,B,3);H=val_func_rbind(val_func_contained,G);J=val_func_compose(H,val_func_last);K=val_func_interval(0,B,1);L=val_func_pair(A,K);M=val_func_sfilter(L,J);N=mval_func_apply(val_func_first,M);O=val_func_fill(I,4,N);return[*map(list,O)]