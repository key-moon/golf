...
def p(A):D=False;A=tuple(map(tuple,A));E=val_func_objects(A,True,D,D);F=val_func_colorfilter(E,0);G=val_func_compose(val_func_last,val_func_center);B=val_func_order(F,G);C=val_func_size(B);H=val_func_interval(0,C,3);I=val_func_rbind(val_func_contained,H);J=val_func_compose(I,val_func_last);K=val_func_interval(0,C,1);L=val_func_pair(B,K);M=val_func_sfilter(L,J);N=mval_func_apply(val_func_first,M);O=val_func_fill(A,4,N);return[*map(list,O)]