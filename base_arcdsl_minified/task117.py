...
def p(A):C=False;B=True;A=tuple(map(tuple,A));J=val_func_objects(A,B,B,B);K=val_func_objects(A,C,B,B);L=val_func_first(K);M=val_func_rbind(val_func_subgrid,A);F=val_func_fork(val_func_equality,val_func_identity,val_func_rot90);N=val_func_compose(F,M);G=val_func_extract(J,N);H=val_func_center(G);O=val_func_subgrid(L,A);D=val_func_hmirror(O);P=val_func_objects(D,C,B,B);Q=val_func_first(P);R=val_func_objects(D,B,B,B);S=val_func_rbind(val_func_subgrid,D);T=val_func_compose(F,S);U=val_func_extract(R,T);V=val_func_center(U);W=val_func_subtract(H,V);X=val_func_shift(Q,W);E=val_func_paint(A,X);Y=val_func_objects(E,C,B,B);Z=val_func_first(Y);a=val_func_subgrid(Z,E);I=val_func_vmirror(a);b=val_func_objects(I,C,B,B);c=val_func_first(b);d=val_func_objects(I,B,B,B);e=val_func_color(G);f=val_func_matcher(val_func_color,e);g=val_func_extract(d,f);h=val_func_center(g);i=val_func_subtract(H,h);j=val_func_shift(c,i);k=val_func_paint(E,j);return[*map(list,k)]