...
def p(I):B=False;A=True;I=tuple(map(tuple,I));J=val_func_objects(I,A,A,A);K=val_func_objects(I,B,A,A);L=val_func_first(K);M=val_func_rbind(val_func_subgrid,I);E=val_func_fork(val_func_equality,val_func_identity,val_func_rot90);N=val_func_compose(E,M);F=val_func_extract(J,N);G=val_func_center(F);O=val_func_subgrid(L,I);C=val_func_hmirror(O);P=val_func_objects(C,B,A,A);Q=val_func_first(P);R=val_func_objects(C,A,A,A);S=val_func_rbind(val_func_subgrid,C);T=val_func_compose(E,S);U=val_func_extract(R,T);V=val_func_center(U);W=val_func_subtract(G,V);X=val_func_shift(Q,W);D=val_func_paint(I,X);Y=val_func_objects(D,B,A,A);Z=val_func_first(Y);a=val_func_subgrid(Z,D);H=val_func_vmirror(a);b=val_func_objects(H,B,A,A);c=val_func_first(b);d=val_func_objects(H,A,A,A);e=val_func_color(F);f=val_func_matcher(val_func_color,e);g=val_func_extract(d,f);h=val_func_center(g);i=val_func_subtract(G,h);j=val_func_shift(c,i);k=val_func_paint(D,j);return[*map(list,k)]