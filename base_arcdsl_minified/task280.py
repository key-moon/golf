...
def val_func_add(A,B):
	...
	return A[0]+B,A[1]+B
def p(A):I=False;A=tuple(map(tuple,A));C=val_func_objects(A,I,I,True);J=val_func_matcher(val_func_first,2);B=val_func_rbind(val_func_sfilter,J);K=val_func_compose(val_func_lowermost,B);L=val_func_compose(val_func_rightmost,B);M=val_func_compose(val_func_uppermost,B);N=val_func_compose(val_func_leftmost,B);O=val_func_fork(val_func_equality,K,val_func_lowermost);P=val_func_fork(val_func_equality,L,val_func_rightmost);Q=val_func_fork(val_func_equality,M,val_func_uppermost);R=val_func_fork(val_func_equality,N,val_func_leftmost);S=val_func_compose(val_func_invert,Q);T=val_func_compose(val_func_invert,R);U=val_func_fork(val_func_add,S,O);V=val_func_fork(val_func_add,T,P);W=val_func_fork(val_func_astuple,U,V);X=val_func_compose(val_func_center,B);D=val_func_fork(val_func_shoot,X,W);Y=mval_func_apply(D,C);Z=val_func_fill(A,2,Y);a=val_func_compose(val_func_vline,D);E=val_func_sfilter(C,a);b=val_func_difference(C,E);F=val_func_chain(val_func_decrement,val_func_minimum,val_func_shape);c=val_func_compose(val_func_increment,F);d=val_func_compose(val_func_invert,F);e=val_func_rbind(val_func_interval,1);G=val_func_fork(e,d,c);f=val_func_lbind(val_func_apply,val_func_toivec);g=val_func_lbind(val_func_apply,val_func_tojvec);h=val_func_lbind(val_func_lbind,val_func_shift);H=val_func_compose(h,D);i=val_func_compose(f,G);j=val_func_compose(g,G);k=val_func_fork(mval_func_apply,H,i);l=val_func_fork(mval_func_apply,H,j);m=mval_func_apply(k,b);n=mval_func_apply(l,E);o=val_func_combine(m,n);p=underval_func_fill(Z,3,o);return[*map(list,p)]