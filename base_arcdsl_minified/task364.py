...
def val_func_add(a,b):
	...
	return a[0]+b,a[1]+b
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_rbind(val_func_subgrid,I);C=val_func_chain(val_func_palette,val_func_trim,B);D=val_func_lbind(val_func_contained,3);E=val_func_compose(D,C);F=val_func_fork(val_func_add,val_func_height,val_func_width);G=val_func_compose(val_func_decrement,F);H=val_func_fork(val_func_equality,val_func_size,G);J=val_func_mfilter(A,E);K=val_func_mfilter(A,H);L=val_func_replace(I,3,6);M=val_func_fill(L,2,J);N=val_func_fill(M,1,K);return[*map(list,N)]