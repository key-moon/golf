...
def p(A):B=True;A=tuple(map(tuple,A));D=val_func_objects(A,B,B,B);E=val_func_interval(1,9,1);F=val_func_apply(val_func_double,E);C=val_func_apply(val_func_decrement,F);G=pval_func_apply(val_func_astuple,C,C);H=val_func_apply(val_func_ulcorner,D);I=val_func_lbind(val_func_shift,G);J=mval_func_apply(I,H);K=val_func_fill(A,4,J);return[*map(list,K)]