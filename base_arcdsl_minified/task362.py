...
def val_func_add(a,b):
	...
	return a[0]+b,a[1]+b
def p(I):I=tuple(map(tuple,I));C=val_func_shape(I);B=val_func_ofcolor(I,5);D=val_func_fill(I,0,B);A=val_func_leastcolor(D);E=val_func_size(B);F=val_func_ofcolor(I,A);G=val_func_rbind(val_func_toobject,I);H=val_func_rbind(val_func_colorcount,A);J=val_func_chain(H,G,val_func_dneighbors);K=val_func_matcher(J,4);L=val_func_extract(F,K);M=val_func_multiply((1,-1),E);N=val_func_add(M,L);O=val_func_canvas(0,C);P=val_func_fork(val_func_combine,val_func_vfrontier,val_func_hfrontier);Q=P(N);R=val_func_fill(O,A,Q);return[*map(list,R)]