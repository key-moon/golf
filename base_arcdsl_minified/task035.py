...
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_sizefilter(A,1);C=val_func_difference(A,B);D=val_func_first(C);E=val_func_rbind(val_func_gravitate,D);F=val_func_compose(val_func_crement,E);G=val_func_fork(val_func_shift,val_func_identity,F);H=mval_func_apply(G,B);J=val_func_paint(I,H);return[*map(list,J)]