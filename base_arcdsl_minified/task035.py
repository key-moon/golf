...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_sizefilter(B,1);D=val_func_difference(B,C);E=val_func_first(D);F=val_func_rbind(val_func_gravitate,E);G=val_func_compose(val_func_crement,F);H=val_func_fork(val_func_shift,val_func_identity,G);I=mval_func_apply(H,C);J=val_func_paint(A,I);return[*map(list,J)]