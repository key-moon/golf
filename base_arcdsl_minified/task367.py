...
def p(I):A=False;I=tuple(map(tuple,I));B=val_func_objects(I,True,A,A);C=val_func_colorfilter(B,0);D=val_func_fork(val_func_equality,val_func_toindices,val_func_backdrop);E=val_func_sfilter(C,D);F=val_func_lbind(mval_func_apply,val_func_dval_func_neighbors);G=val_func_chain(F,val_func_corners,val_func_outbox);H=val_func_fork(val_func_difference,G,val_func_outbox);J=val_func_ofcolor(I,5);K=val_func_rbind(val_func_intersection,J);L=val_func_matcher(val_func_size,0);M=val_func_chain(L,K,H);N=val_func_mfilter(E,M);O=val_func_fill(I,4,N);return[*map(list,O)]