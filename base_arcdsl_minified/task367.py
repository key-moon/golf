...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_objects(A,True,B,B);D=val_func_colorfilter(C,0);E=val_func_fork(val_func_equality,val_func_toindices,val_func_backdrop);F=val_func_sfilter(D,E);G=val_func_lbind(mval_func_apply,val_func_dval_func_neighbors);H=val_func_chain(G,val_func_corners,val_func_outbox);I=val_func_fork(val_func_difference,H,val_func_outbox);J=val_func_ofcolor(A,5);K=val_func_rbind(val_func_intersection,J);L=val_func_matcher(val_func_size,0);M=val_func_chain(L,K,I);N=val_func_mfilter(F,M);O=val_func_fill(A,4,N);return[*map(list,O)]