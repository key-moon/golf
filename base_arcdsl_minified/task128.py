...
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_merge(A);C=val_func_cover(I,B);D=val_func_chain(val_func_toivec,val_func_invert,val_func_height);E=val_func_fork(val_func_shift,val_func_identity,D);F=mval_func_apply(E,A);G=val_func_paint(C,F);return[*map(list,G)]