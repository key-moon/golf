...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_merge(B);D=val_func_cover(A,C);E=val_func_chain(val_func_toivec,val_func_invert,val_func_height);F=val_func_fork(val_func_shift,val_func_identity,E);G=mval_func_apply(F,B);H=val_func_paint(D,G);return[*map(list,H)]