...
def val_func_subtract(a,b):
	...
	return a[0]-b,a[1]-b
def p(I):I=tuple(map(tuple,I));A=val_func_leastcolor(I);B=val_func_ofcolor(I,A);C=val_func_center(B);E=val_func_ulcorner(B);F=val_func_subtract(C,E);G=val_func_multiply(-1,9);H=val_func_interval(0,9,1);J=val_func_interval(0,G,-1);D=val_func_lbind(val_func_multiply,F);K=val_func_apply(D,H);L=val_func_apply(D,J);M=val_func_pair(K,L);N=mval_func_apply(val_func_box,M);O=val_func_shift(N,C);P=val_func_fill(I,A,O);return[*map(list,P)]