...
def val_func_add(A,B):
	...
	return A[0]+B,A[1]+B
def p(A):C=True;A=tuple(map(tuple,A));E=val_func_objects(A,C,C,C);B=val_func_ofcolor(A,2);F=val_func_vmirror(B);G=val_func_height(B);H=val_func_width(B);I=val_func_toivec(G);J=val_func_tojvec(H);K=val_func_add(J,(0,2));L=val_func_add(I,(2,0));M=val_func_shift(F,K);D=val_func_fill(A,2,M);N=val_func_ofcolor(D,2);O=val_func_hmirror(N);P=val_func_shift(O,L);Q=val_func_fill(D,2,P);R=val_func_size(E);S=val_func_greater(R,4);T=val_func_branch(S,A,Q);return[*map(list,T)]