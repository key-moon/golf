...
def val_func_add(a,b):
	...
	return a[0]+b,a[1]+b
def p(I):B=True;I=tuple(map(tuple,I));D=val_func_objects(I,B,B,B);A=val_func_ofcolor(I,2);E=val_func_vmirror(A);F=val_func_height(A);G=val_func_width(A);H=val_func_toivec(F);J=val_func_tojvec(G);K=val_func_add(J,(0,2));L=val_func_add(H,(2,0));M=val_func_shift(E,K);C=val_func_fill(I,2,M);N=val_func_ofcolor(C,2);O=val_func_hmirror(N);P=val_func_shift(O,L);Q=val_func_fill(C,2,P);R=val_func_size(D);S=val_func_greater(R,4);T=val_func_branch(S,I,Q);return[*map(list,T)]