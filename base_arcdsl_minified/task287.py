...
def pval_func_apply(function,a,b):return tuple(function(A,B)for(A,B)in zip(a,b))
...
def p(I):I=tuple(map(tuple,I));A=val_func_replace(I,4,0);D=val_func_dmirror(A);E=pval_func_apply(val_func_pair,A,D);B=val_func_lbind(val_func_apply,val_func_maximum);C=val_func_apply(B,E);F=val_func_cmirror(C);G=pval_func_apply(val_func_pair,C,F);H=val_func_apply(B,G);J=val_func_cmirror(H);return[*map(list,J)]