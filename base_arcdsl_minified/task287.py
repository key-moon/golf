...
def p(A):A=tuple(map(tuple,A));B=val_func_replace(A,4,0);E=val_func_dmirror(B);F=pval_func_apply(val_func_pair,B,E);C=val_func_lbind(val_func_apply,val_func_maximum);D=val_func_apply(C,F);G=val_func_cmirror(D);H=pval_func_apply(val_func_pair,D,G);I=val_func_apply(C,H);J=val_func_cmirror(I);return[*map(list,J)]