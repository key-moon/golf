...
def val_func_add(a,b):
	...
	return a[0]+b,a[1]+b
def p(I):I=tuple(map(tuple,I));E=val_func_fgpartition(I);F=val_func_objects(I,True,False,True);B=val_func_order(E,val_func_height);G=val_func_last(B);H=val_func_remove(G,B);J=val_func_last(H);K=val_func_color(J);C=val_func_val_func_colorfilter(F,K);L=val_func_power(val_func_outbox,2);M=val_func_rbind(val_func_toobject,I);N=val_func_mostval_func_color(I);O=val_func_lbind(val_func_remove,N);P=val_func_chain(val_func_size,O,val_func_palette);Q=val_func_chain(P,M,L);D=val_func_argmax(C,Q);R=val_func_ulcorner(D);A=val_func_shape(D);S=val_func_subtract(R,A);T=val_func_decrement(S);U=val_func_multiply(A,3);V=val_func_add(U,(2,2));W=val_func_crop(I,T,V);X=val_func_asobject(W);Y=val_func_apply(val_func_ulcorner,C);Z=val_func_increment(A);a=val_func_rbind(val_func_subtract,Z);b=val_func_apply(a,Y);c=val_func_lbind(val_func_shift,X);d=mval_func_apply(c,b);e=val_func_paint(I,d);return[*map(list,e)]