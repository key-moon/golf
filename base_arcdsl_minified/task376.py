...
def p(I):I=tuple(map(tuple,I));D=val_func_height(I);A=val_func_width(I);B=val_func_decrement(D);E=val_func_astuple(B,A);F=val_func_crop(I,(0,0),E);G=val_func_hmirror(F);C=val_func_vconcat(I,G);H=val_func_double(B);J=val_func_astuple(H,A);K=val_func_crop(C,(1,0),J);L=val_func_vconcat(C,K);return[*map(list,L)]