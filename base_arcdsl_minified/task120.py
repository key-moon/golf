...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_compose(val_func_backdrop,val_func_inbox);D=mval_func_apply(C,B);E=val_func_fill(A,8,D);return[*map(list,E)]