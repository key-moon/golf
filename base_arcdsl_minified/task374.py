...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_replace(A,5,4);D=val_func_argmin(B,val_func_size);E=val_func_argmax(B,val_func_size);F=val_func_fill(C,1,E);G=val_func_fill(F,2,D);return[*map(list,G)]