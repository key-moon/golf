...
def p(A):A=tuple(map(tuple,A));E=val_func_objects(A,True,False,True);B=val_func_replace(A,8,0);C=val_func_leastcolor(B);F=val_func_replace(B,C,0);G=val_func_leastcolor(F);D=val_func_merge(E);H=val_func_backdrop(D);I=val_func_box(D);J=val_func_fill(A,C,H);K=val_func_fill(J,G,I);return[*map(list,K)]