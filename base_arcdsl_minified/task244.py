...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_compress(A);D=val_func_objects(A,True,B,B);E=val_func_vmirror(C);F=val_func_valmin(D,val_func_width);G=val_func_downscale(E,F);return[*map(list,G)]