...
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_argmax(A,val_func_size);C=val_func_argmin(A,val_func_size);D=val_func_replace(I,5,0);E=val_func_recolor(1,B);F=val_func_recolor(2,C);G=val_func_combine(E,F);H=val_func_paint(D,G);return[*map(list,H)]