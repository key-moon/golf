...
def p(A):A=tuple(map(tuple,A));C=val_func_ofcolor(A,8);B=val_func_subgrid(C,A);D=val_func_replace(A,8,0);E=val_func_replace(D,1,0);F=val_func_compress(E);G=val_func_width(B);H=val_func_halve(G);I=val_func_upscale(F,H);J=val_func_ofcolor(B,0);K=val_func_fill(I,0,J);return[*map(list,K)]