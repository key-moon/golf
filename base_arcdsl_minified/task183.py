...
def p(I):I=tuple(map(tuple,I));B=val_func_ofcolor(I,8);A=val_func_subgrid(B,I);C=val_func_replace(I,8,0);D=val_func_replace(C,1,0);E=val_func_compress(D);F=val_func_width(A);G=val_func_halve(F);H=val_func_upscale(E,G);J=val_func_ofcolor(A,0);K=val_func_fill(H,0,J);return[*map(list,K)]