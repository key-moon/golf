...
def p(I):I=tuple(map(tuple,I));A=val_func_leastcolor(I);B=val_func_objects(I,True,False,True);C=val_func_colorfilter(B,A);D=val_func_merge(C);E=val_func_delta(D);F=val_func_first(E);G=val_func_neighbors(F);H=val_func_fill(I,4,G);return[*map(list,H)]