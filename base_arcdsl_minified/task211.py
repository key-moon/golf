...
def p(I):I=tuple(map(tuple,I));B=val_func_vmirror(I);A=val_func_hconcat(B,I);C=val_func_hmirror(A);D=val_func_vconcat(A,C);E=val_func_vconcat(D,A);F=val_func_hmirror(E);return[*map(list,F)]