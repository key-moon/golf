...
def p(A):D=False;A=tuple(map(tuple,A));B=val_func_objects(A,True,D,D);C=val_func_argmin(B,val_func_size);E=val_func_rbind(val_func_adjacent,C);F=val_func_remove(C,B);G=val_func_extract(F,E);H=val_func_color(G);I=val_func_canvas(H,(1,1));return[*map(list,I)]