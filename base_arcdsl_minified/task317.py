...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_apply(val_func_outbox,B);D=mval_func_apply(val_func_backdrop,C);E=val_func_fill(A,1,D);return[*map(list,E)]