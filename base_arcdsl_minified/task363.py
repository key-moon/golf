...
def p(I):I=tuple(map(tuple,I));B=val_func_ofcolor(I,2);A=val_func_recolor(0,B);C=val_func_normalize(A);D=val_func_occurrences(I,A);E=val_func_lbind(val_func_shift,C);F=val_func_apply(E,D);G=val_func_astuple(1,3);H=val_func_astuple(5,1);J=val_func_astuple(2,6);K=val_func_initset(G);L=val_func_insert(H,K);M=val_func_insert(J,L);N=val_func_rbind(val_func_contained,M);O=val_func_chain(val_func_flip,N,val_func_ulcorner);P=val_func_sfilter(F,O);Q=val_func_merge(P);R=val_func_recolor(2,Q);S=val_func_paint(I,R);return[*map(list,S)]