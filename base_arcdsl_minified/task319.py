...
def p(I):I=tuple(map(tuple,I));A=val_func_vmirror(I);D=val_func_fgpartition(A);B=val_func_order(D,val_func_size);C=val_func_last(B);E=val_func_remove(C,B);F=val_func_compose(val_func_toindices,val_func_normalize);G=val_func_rbind(val_func_upscale,2);H=val_func_chain(val_func_toindices,G,val_func_normalize);J=F(C);K=val_func_rbind(val_func_intersection,J);L=val_func_chain(val_func_size,K,H);M=val_func_argmax(E,L);N=val_func_subgrid(M,A);O=val_func_vmirror(N);return[*map(list,O)]