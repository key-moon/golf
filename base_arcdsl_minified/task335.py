...
def p(A):A=tuple(map(tuple,A));E=val_func_ofcolor(A,8);F=val_func_ofcolor(A,2);B=val_func_first(E);C=val_func_first(F);G=val_func_last(B);H=val_func_first(C);D=val_func_astuple(H,G);I=val_func_connect(D,B);J=val_func_connect(D,C);K=val_func_combine(I,J);L=val_func_underfill(A,4,K);return[*map(list,L)]