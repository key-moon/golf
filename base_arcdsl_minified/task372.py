...
def p(A):A=tuple(map(tuple,A));B=val_func_tophalf(A);C=val_func_bottomhalf(A);D=val_func_objects(B,True,False,True);E=val_func_merge(D);F=val_func_paint(C,E);return[*map(list,F)]