...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_objects(A,True,B,B);D=val_func_colorfilter(C,5);E=val_func_compose(val_func_backdrop,val_func_inbox);F=mval_func_apply(E,D);G=val_func_fill(A,2,F);return[*map(list,G)]