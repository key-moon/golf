...
def p(I):C=False;I=tuple(map(tuple,I));D=val_func_objects(I,True,C,C);A=val_func_colorfilter(D,4);B=val_func_compose(val_func_backdrop,val_func_inbox);E=val_func_argmin(A,val_func_size);F=val_func_argmax(A,val_func_size);G=B(E);H=B(F);J=val_func_fill(I,1,G);K=val_func_fill(J,2,H);return[*map(list,K)]