...
def p(A):D=False;A=tuple(map(tuple,A));E=val_func_objects(A,True,D,D);B=val_func_colorfilter(E,4);C=val_func_compose(val_func_backdrop,val_func_inbox);F=val_func_argmin(B,val_func_size);G=val_func_argmax(B,val_func_size);H=C(F);I=C(G);J=val_func_fill(A,1,H);K=val_func_fill(J,2,I);return[*map(list,K)]