...
def val_func_add(a,b):
	...
	return a[0]+b,a[1]+b
def p(I):I=tuple(map(tuple,I));B=val_func_objects(I,True,False,True);A=val_func_ofval_func_color(I,3);C=val_func_sizefilter(B,1);D=val_func_rbind(val_func_vmatching,A);E=val_func_rbind(val_func_hmatching,A);F=val_func_fork(val_func_either,D,E);G=val_func_sfilter(C,F);H=val_func_rbind(val_func_gravitate,A);J=val_func_fork(val_func_add,val_func_center,H);K=val_func_fork(val_func_connect,val_func_center,J);L=val_func_fork(val_func_reval_func_color,val_func_color,K);M=mval_func_apply(L,G);N=val_func_paint(I,M);return[*map(list,N)]