...
def val_func_add(A,B):
	...
	return A[0]+B,A[1]+B
def p(A):A=tuple(map(tuple,A));C=val_func_objects(A,True,False,True);B=val_func_ofval_func_color(A,3);D=val_func_sizefilter(C,1);E=val_func_rbind(val_func_vmatching,B);F=val_func_rbind(val_func_hmatching,B);G=val_func_fork(val_func_either,E,F);H=val_func_sfilter(D,G);I=val_func_rbind(val_func_gravitate,B);J=val_func_fork(val_func_add,val_func_center,I);K=val_func_fork(val_func_connect,val_func_center,J);L=val_func_fork(val_func_reval_func_color,val_func_color,K);M=mval_func_apply(L,H);N=val_func_paint(A,M);return[*map(list,N)]