...
def p(A):C=False;A=tuple(map(tuple,A));D=val_func_shape(A);E=val_func_objects(A,True,C,C);F=val_func_colorfilter(E,0);G=val_func_apply(val_func_toindices,F);H=val_func_lbind(val_func_lbind,val_func_contained);I=val_func_lbind(val_func_extract,G);B=val_func_compose(I,H);J=val_func_decrement(D);K=val_func_astuple(5,5);L=B((0,0));M=B(J);N=B(K);O=val_func_fill(A,1,L);P=val_func_fill(O,3,M);Q=val_func_fill(P,2,N);return[*map(list,Q)]