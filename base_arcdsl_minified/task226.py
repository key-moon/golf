...
def p(I):B=False;I=tuple(map(tuple,I));C=val_func_shape(I);D=val_func_objects(I,True,B,B);E=val_func_colorfilter(D,0);F=val_func_apply(val_func_toindices,E);G=val_func_lbind(val_func_lbind,val_func_contained);H=val_func_lbind(val_func_extract,F);A=val_func_compose(H,G);J=val_func_decrement(C);K=val_func_astuple(5,5);L=A((0,0));M=A(J);N=A(K);O=val_func_fill(I,1,L);P=val_func_fill(O,3,M);Q=val_func_fill(P,2,N);return[*map(list,Q)]