...
def val_func_subtract(a,b):
	...
	return a[0]-b,a[1]-b
def p(I):I=tuple(map(tuple,I));C=val_func_objects(I,True,False,True);A=val_func_totuple(C);D=val_func_compose(val_func_increment,val_func_ulcorner);E=val_func_compose(val_func_decrement,val_func_lrcorner);B=val_func_apply(D,A);F=val_func_apply(E,A);G=val_func_pval_func_apply(val_func_connect,B,F);H=val_func_apply(val_func_last,B);J=val_func_compose(val_func_last,val_func_first);K=val_func_power(val_func_last,2);L=val_func_fork(val_func_subtract,J,K);M=val_func_compose(val_func_even,L);N=val_func_lbind(val_func_rbind,val_func_astuple);O=val_func_lbind(val_func_compose,M);P=val_func_compose(O,N);Q=val_func_fork(val_func_sfilter,val_func_first,P);R=val_func_pair(G,H);S=mval_func_apply(Q,R);T=val_func_fill(I,0,S);return[*map(list,T)]