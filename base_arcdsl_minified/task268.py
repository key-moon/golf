...
def val_func_subtract(A,B):
	...
	return A[0]-B,A[1]-B
def p(A):A=tuple(map(tuple,A));G=val_func_objects(A,True,False,True);B=mval_func_apply(val_func_toindices,G);H=val_func_box(B);C=val_func_difference(H,B);E=val_func_delta(B);I=val_func_position(E,C);J=val_func_interval(0,9,1);K=val_func_lbind(val_func_multiply,I);L=val_func_apply(K,J);M=val_func_lbind(val_func_shift,C);F=mval_func_apply(M,L);N=val_func_fill(A,4,E);D=val_func_fill(N,4,F);O=val_func_corners(C);P=val_func_ofcolor(D,0);Q=val_func_rbind(val_func_toobject,D);R=val_func_rbind(val_func_colorcount,0);S=val_func_chain(R,Q,val_func_dval_func_neighbors);T=val_func_matcher(S,2);U=val_func_rbind(val_func_adjacent,B);V=val_func_rbind(val_func_adjacent,F);W=val_func_fork(val_func_both,U,V);X=val_func_compose(W,val_func_initset);Y=val_func_sfilter(P,T);Z=val_func_sfilter(Y,X);a=val_func_product(O,Z);b=val_func_fork(val_func_subtract,val_func_last,val_func_first);c=val_func_fork(val_func_shoot,val_func_first,b);d=mval_func_apply(c,a);e=val_func_fill(D,4,d);return[*map(list,e)]