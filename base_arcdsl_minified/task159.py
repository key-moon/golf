...
def val_func_divide(A,B):
	...
	return A[0]//B,A[1]//B
def p(A):B=True;A=tuple(map(tuple,A));D=val_func_objects(A,B,B,B);E=val_func_ofcolor(A,2);F=val_func_argmin(D,val_func_size);C=val_func_subgrid(E,A);G=val_func_width(C);H=val_func_divide(G,3);I=val_func_upscale(F,H);J=val_func_normalize(I);K=val_func_shift(J,(1,1));L=val_func_paint(C,K);return[*map(list,L)]