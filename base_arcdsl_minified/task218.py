...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_objects(A,B,B,True);D=val_func_first(C);E=val_func_subgrid(D,A);F=val_func_dedupe(E);G=val_func_rot90(F);H=val_func_dedupe(G);I=val_func_rot270(H);return[*map(list,I)]