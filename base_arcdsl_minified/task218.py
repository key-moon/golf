...
def p(I):A=False;I=tuple(map(tuple,I));B=val_func_objects(I,A,A,True);C=val_func_first(B);D=val_func_subgrid(C,I);E=val_func_dedupe(D);F=val_func_rot90(E);G=val_func_dedupe(F);H=val_func_rot270(G);return[*map(list,H)]