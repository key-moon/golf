...
def p(A):D=False;A=tuple(map(tuple,A));B=val_func_objects(A,D,D,True);C=val_func_colorfilter(B,5);E=val_func_first(C);F=val_func_center(E);G=val_func_difference(B,C);H=val_func_first(G);I=val_func_normalize(H);J=val_func_shift(I,F);K=val_func_shift(J,(-1,-1));L=val_func_paint(A,K);return[*map(list,L)]