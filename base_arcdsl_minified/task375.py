...
def p(A):A=tuple(map(tuple,A));B=val_func_width(A);C=val_func_shoot((0,0),(1,1));D=val_func_decrement(B);E=val_func_tojvec(D);F=val_func_shoot(E,(1,-1));G=val_func_combine(C,F);H=val_func_fill(A,0,G);return[*map(list,H)]