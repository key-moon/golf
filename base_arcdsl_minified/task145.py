...
def p(A):C=False;A=tuple(map(tuple,A));B=val_func_objects(A,True,C,C);D=val_func_colorfilter(B,0);E=val_func_argmax(B,val_func_size);F=val_func_valmin(B,val_func_size);G=val_func_val_func_sizefilter(D,F);H=val_func_recolor(1,E);I=val_func_merge(G);J=val_func_paint(A,H);K=val_func_fill(J,8,I);return[*map(list,K)]