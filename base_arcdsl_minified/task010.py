def val_func_papply(A,B,C):return tuple(A(B,C)for(B,C)in zip(B,C))
...
def mval_func_papply(A,B,C):return val_func_merge(val_func_papply(A,B,C))
...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_totuple(B);D=val_func_order(B,val_func_height);E=val_func_size(C);F=val_func_interval(E,0,-1);G=mval_func_papply(val_func_recolor,F,D);H=val_func_paint(A,G);return[*map(list,H)]