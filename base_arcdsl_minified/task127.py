...
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_sizefilter(A,1);C=val_func_compose(val_func_neighbors,val_func_center);D=val_func_fork(val_func_reval_func_color,val_func_color,C);E=mval_func_apply(D,B);F=val_func_paint(I,E);G=val_func_replace(F,1,6);H=val_func_replace(G,2,7);J=val_func_replace(H,3,8);K=val_func_replace(J,4,9);return[*map(list,K)]