...
def p(A):A=tuple(map(tuple,A));B=val_func_fgpartition(A);C=val_func_fork(reval_func_color,val_func_color,val_func_backdrop);D=val_func_apply(C,B);E=val_func_mfilter(D,val_func_hline);F=val_func_paint(A,E);return[*map(list,F)]