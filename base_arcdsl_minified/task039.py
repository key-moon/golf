...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,False,True,True);C=val_func_first(B);D=val_func_subgrid(C,A);E=val_func_lefthalf(D);F=val_func_tophalf(E);return[*map(list,F)]