...
def p(A):B=True;A=tuple(map(tuple,A));D=val_func_objects(A,B,B,B);C=val_func_apply(val_func_toindices,D);E=val_func_fork(val_func_either,val_func_vline,val_func_hline);F=val_func_sfilter(C,E);G=val_func_difference(C,F);H=val_func_fork(val_func_equality,val_func_identity,val_func_box);I=val_func_mfilter(G,H);J=val_func_fill(A,3,I);return[*map(list,J)]