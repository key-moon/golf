...
def p(I):I=tuple(map(tuple,I));B=val_func_objects(I,True,False,True);A=val_func_sizefilter(B,1);C=val_func_product(A,A);D=val_func_fork(val_func_vmatching,val_func_first,val_func_last);E=val_func_fork(val_func_hmatching,val_func_first,val_func_last);F=val_func_fork(val_func_either,D,E);G=val_func_sfilter(C,F);H=val_func_compose(val_func_center,val_func_first);J=val_func_compose(val_func_center,val_func_last);K=val_func_fork(val_func_connect,H,J);L=val_func_chain(val_func_initset,val_func_center,K);M=val_func_compose(val_func_color,val_func_first);N=val_func_fork(val_func_reval_func_color,M,L);O=mval_func_apply(N,G);P=val_func_paint(I,O);return[*map(list,P)]