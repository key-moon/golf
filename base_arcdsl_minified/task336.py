...
def p(A):A=tuple(map(tuple,A));B=val_func_ofcolor(A,5);E=val_func_delta(B);F=val_func_fill(A,8,E);C=val_func_box(B);D=val_func_difference(C,B);G=val_func_position(C,D);H=val_func_first(D);I=val_func_shoot(H,G);J=val_func_fill(F,8,I);return[*map(list,J)]