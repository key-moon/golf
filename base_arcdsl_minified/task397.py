...
def val_func_add(A,B):
	...
	return A[0]+B,A[1]+B
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,False,True,True);C=val_func_rbind(val_func_add,(1,0));D=val_func_compose(C,val_func_llcorner);E=val_func_compose(val_func_toivec,val_func_numcolors);F=val_func_fork(val_func_add,val_func_lrcorner,E);G=val_func_fork(val_func_astuple,D,F);H=val_func_compose(val_func_box,G);I=mval_func_apply(H,B);J=val_func_fill(A,3,I);return[*map(list,J)]