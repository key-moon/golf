...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_objects(A,True,B,B);D=val_func_argmax(C,val_func_height);E=val_func_subgrid(D,A);F=val_func_trim(E);return[*map(list,F)]