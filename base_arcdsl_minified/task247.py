...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_valmax(B,val_func_size);D=val_func_val_func_sizefilter(B,C);E=val_func_order(D,val_func_leftmost);F=val_func_apply(val_func_color,E);G=val_func_astuple(1,C);H=val_func_rbind(val_func_canvas,G);I=val_func_apply(H,F);J=val_func_merge(I);K=val_func_dmirror(J);return[*map(list,K)]