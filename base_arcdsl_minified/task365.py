...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_width(A);D=val_func_astuple(9,C);E=val_func_canvas(0,D);F=val_func_vconcat(A,E);G=val_func_objects(F,B,B,True);H=val_func_rbind(val_func_colorcount,2);I=val_func_argmax(G,H);J=val_func_subgrid(I,A);return[*map(list,J)]