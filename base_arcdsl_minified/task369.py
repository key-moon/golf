...
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_sizefilter(A,1);C=val_func_sizefilter(A,2);D=val_func_merge(B);E=val_func_fill(I,3,D);F=val_func_merge(C);G=val_func_fill(E,2,F);H=val_func_replace(G,0,1);return[*map(list,H)]