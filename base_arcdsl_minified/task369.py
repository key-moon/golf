...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_sizefilter(B,1);D=val_func_sizefilter(B,2);E=val_func_merge(C);F=val_func_fill(A,3,E);G=val_func_merge(D);H=val_func_fill(F,2,G);I=val_func_replace(H,0,1);return[*map(list,I)]