...
def p(A):A=tuple(map(tuple,A));C=val_func_objects(A,True,False,True);D=val_func_ofval_func_color(A,8);B=val_func_sizefilter(C,1);E=val_func_apply(val_func_initset,D);F=val_func_lbind(val_func_argmin,E);G=val_func_lbind(val_func_rbind,val_func_manhattan);H=val_func_compose(F,G);I=val_func_fork(val_func_reval_func_color,val_func_color,H);J=mval_func_apply(I,B);K=val_func_merge(B);L=val_func_cover(A,K);M=val_func_paint(L,J);return[*map(list,M)]