...
def p(A):A=tuple(map(tuple,A));B=val_func_fgpartition(A);C=val_func_fork(reval_func_color,val_func_color,val_func_backdrop);D=mval_func_apply(C,B);E=val_func_paint(A,D);return[*map(list,E)]