...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_shape(A);D=val_func_objects(A,True,B,B);E=val_func_argmax(D,val_func_size);F=val_func_canvas(5,C);G=val_func_paint(F,E);return[*map(list,G)]