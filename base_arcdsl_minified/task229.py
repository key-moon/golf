...
def p(I):A=False;I=tuple(map(tuple,I));B=val_func_shape(I);C=val_func_objects(I,True,A,A);D=val_func_argmax(C,val_func_size);E=val_func_canvas(5,B);F=val_func_paint(E,D);return[*map(list,F)]