...
def p(A):A=tuple(map(tuple,A));B=val_func_crop(A,(0,0),(3,3));C=val_func_ofcolor(B,0);D=val_func_asindices(B);E=val_func_difference(D,C);F=val_func_normalize(E);G=val_func_objects(A,True,False,True);H=val_func_compose(val_func_toindices,val_func_normalize);I=val_func_matcher(H,F);J=val_func_mfilter(G,I);K=val_func_fill(A,5,J);L=val_func_asobject(B);M=val_func_paint(K,L);return[*map(list,M)]