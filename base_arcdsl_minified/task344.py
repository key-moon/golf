...
def p(A):C=False;A=tuple(map(tuple,A));D=val_func_objects(A,C,C,True);E=val_func_sizefilter(D,2);F=val_func_lbind(val_func_contained,2);G=val_func_compose(F,val_func_palette);B=val_func_mfilter(E,G);H=val_func_cover(A,B);I=val_func_matcher(val_func_first,3);J=val_func_sfilter(B,I);K=val_func_fill(H,8,J);return[*map(list,K)]