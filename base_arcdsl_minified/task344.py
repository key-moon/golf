...
def p(I):B=False;I=tuple(map(tuple,I));C=val_func_objects(I,B,B,True);D=val_func_sizefilter(C,2);E=val_func_lbind(val_func_contained,2);F=val_func_compose(E,val_func_palette);A=val_func_mfilter(D,F);G=val_func_cover(I,A);H=val_func_matcher(val_func_first,3);J=val_func_sfilter(A,H);K=val_func_fill(G,8,J);return[*map(list,K)]