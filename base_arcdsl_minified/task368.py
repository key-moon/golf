...
def p(A):D=False;A=tuple(map(tuple,A));B=val_func_objects(A,D,D,True);C=val_func_argmax(B,val_func_numcolors);E=val_func_normalize(C);F=val_func_remove(C,B);G=val_func_apply(val_func_ulcorner,F);H=val_func_lbind(val_func_shift,E);I=mval_func_apply(H,G);J=val_func_paint(A,I);return[*map(list,J)]