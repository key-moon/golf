...
def p(A):A=tuple(map(tuple,A));C=val_func_upscale(A,3);D=val_func_hconcat(A,A);B=val_func_hconcat(D,A);E=val_func_vconcat(B,B);F=val_func_vconcat(E,B);G=val_func_ofcolor(C,0);H=val_func_ofcolor(C,1);I=val_func_combine(G,H);J=val_func_fill(F,0,I);return[*map(list,J)]