...
def p(I):I=tuple(map(tuple,I));A=val_func_objects(I,True,False,True);B=val_func_val_func_sizefilter(A,1);C=val_func_argmax(A,val_func_size);D=val_func_outbox(C);E=val_func_corners(D);F=val_func_lbind(val_func_rbind,val_func_manhattan);G=val_func_lbind(val_func_argmax,B);H=val_func_chain(G,F,val_func_initset);J=val_func_compose(val_func_color,H);K=val_func_fork(val_func_astuple,J,val_func_identity);L=val_func_apply(K,E);M=val_func_merge(B);N=val_func_cover(I,M);O=val_func_paint(N,L);return[*map(list,O)]