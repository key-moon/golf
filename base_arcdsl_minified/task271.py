...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_objects(A,B,B,True);D=val_func_rbind(val_func_colorcount,1);E=val_func_argmax(C,D);F=val_func_subgrid(E,A);return[*map(list,F)]