...
def val_func_multiply(A,B):
	...
	return A[0]*B,A[1]*B
def p(A):B=True;A=tuple(map(tuple,A));D=val_func_objects(A,B,B,B);C=val_func_first(D);E=val_func_interval(-2,4,1);F=val_func_lbind(val_func_shift,C);G=val_func_height(C);H=val_func_rbind(val_func_multiply,G);I=val_func_apply(H,E);J=val_func_apply(val_func_toivec,I);K=mval_func_apply(F,J);L=val_func_paint(A,K);return[*map(list,L)]