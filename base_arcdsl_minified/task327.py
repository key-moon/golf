...
def p(A):A=tuple(map(tuple,A));B=val_func_objects(A,True,False,True);C=val_func_rbind(val_func_shoot,(1,1));D=val_func_compose(C,val_func_center);E=val_func_fork(reval_func_color,val_func_color,D);F=mval_func_apply(E,B);G=val_func_astuple(6,6);H=val_func_canvas(0,G);I=val_func_paint(H,F);return[*map(list,I)]