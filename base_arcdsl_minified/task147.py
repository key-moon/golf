...
def p(I):B=False;I=tuple(map(tuple,I));C=val_func_objects(I,True,B,B);A=val_func_colorfilter(C,3);D=val_func_sizefilter(A,1);E=val_func_difference(A,D);F=val_func_merge(E);G=val_func_fill(I,8,F);return[*map(list,G)]