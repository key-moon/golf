...
def p(A):C=False;A=tuple(map(tuple,A));D=val_func_objects(A,True,C,C);B=val_func_colorfilter(D,3);E=val_func_sizefilter(B,1);F=val_func_difference(B,E);G=val_func_merge(F);H=val_func_fill(A,8,G);return[*map(list,H)]