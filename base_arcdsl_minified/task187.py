...
def p(A):B=False;A=tuple(map(tuple,A));C=val_func_objects(A,True,B,B);D=val_func_rbind(val_func_bordering,A);E=val_func_compose(val_func_flip,D);F=val_func_mfilter(C,E);G=val_func_fill(A,2,F);H=val_func_replace(G,0,3);return[*map(list,H)]