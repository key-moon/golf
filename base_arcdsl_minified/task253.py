...
def p(A):A=tuple(map(tuple,A));C=val_func_objects(A,True,False,True);D=val_func_lbind(val_func_index,A);E=val_func_matcher(D,0);F=val_func_lbind(val_func_extract,C);G=val_func_rbind(val_func_subgrid,A);H=val_func_lbind(val_func_compose,E);B=val_func_chain(G,F,H);I=B(val_func_ulcorner);J=B(val_func_urcorner);K=B(val_func_llcorner);L=B(val_func_lrcorner);M=val_func_hconcat(L,K);N=val_func_hconcat(J,I);O=val_func_vconcat(M,N);return[*map(list,O)]