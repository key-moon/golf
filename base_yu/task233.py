...
def p(g):
 u=[s*1for s in g]
 for _ in range(80):
  u=[*map(list,zip(*u[("2, "*7in str(u[-1]))-2::-1]))]
 for _ in range(8):
  for i in range(len(g)-2):
   for j in range(len(g[0])-2):
    for y in range(len(u)-2):
     for x in range(len(u[0])-2):
      if all(g[i+k][j+m]==2 if u[y+k][x+m]==0 else g[i+k][j+m]|2!=2 for k in range(3)for m in range(3))*(y<1 or 0<u[y-1][x+1])*(y+4>len(u) or 0<u[y+3][x+1]):
       for k in range(3):
        for m in range(3):
         u[y+k][x+m]=g[i+k][j+m]
         g[i+k][j+m]=0
