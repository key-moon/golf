# best: 103(ox jam) / others: 107(jailctf merger), 114(4atj sisyphus luke Seek mukundan), 120(THUNDER THUNDER), 135(intgrah jimboko awu macaque sammyuri), 144(jacekw Potatoman nauti natte)
# ================================================ 103 ================================================
...
# lambda g,c=-7:c*g or p([[(v in[0,*s,0][i:i+3:2]*(5<sum(g,g[0]).count(v)))*v for i,v in enumerate(s)]for s in zip(*g)],c+1)
# lambda g,c=-7:c*g or p([[sum(g,[0,*s][i:i+3]*40).count(s[i])//85*s[i]for i in range(16)]for s in zip(*g)],c+1)
# lambda g,c=-7:c*g or p([[v*(85<sum(g,[0,*s][i:i+3]*40).count(v))for i,v in enumerate(s)]for s in zip(*g)],c+1)
p=lambda g,c=-7:c*g or p([[v*(45<sum(g,t*40).count(v))for*t,v in zip([0,*s],s[1:]*2,s)]for s in zip(*g)],c+1)
