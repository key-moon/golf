...
# p=lambda g,c=-5:c*g or p([*zip(*[[(s[i]in[0,*s,0][i:i+3:2]and(c<0 or len({*g})==2 or s[i]in(2,9)))*s[i] for i in range(16)]for s in g])],c+1)
p=lambda g,c=-5:c*g or p([*zip(*[[(v in[0,*s,0][i:i+3:2]and(c<0 or len({*g})==2 or v in(2,9)))*v for i,v in enumerate(s)]for s in g])],c+1)
