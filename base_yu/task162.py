...
# lambda g,c=-3:c*g or p(eval(re.sub(("(.{55})0, 0, 0"*3)[7:],"1,1,1\\1 1,1,1\\2 1,1,1",str(g))),c+1)
# lambda g:eval(r'(g:=eval(re.sub("(.{55})?0, 0, 0"*3,"\\1 1,1,1\\2 1,1,1\\3 1,1,1",str(g)))),'*3)[2]
# lambda g,c=-3:c*g or p(eval(re.sub("(.{55})?0, 0, 0"*3," 1,1,1\\".join("01234")[7:-2],str(g))),c+1)
# lambda g,c=-3:c*g or p(eval(re.sub("(.{55})?0, 0, 0"*3,"%s 1,1,1"*3%("\\1","\\2","\\3"),str(g))),c+1)
import re
