...
def solve_447fd412(I):
	...
	if not b:return I
	p=[list(A)for A in I];[p[A].__setitem__(B,C)for(C,(A,B))in set().union(*b)if 0<=A<D and 0<=B<E];return tuple(tuple(A)for A in p)
def p(g):return[list(A)for A in solve_447fd412(g)]