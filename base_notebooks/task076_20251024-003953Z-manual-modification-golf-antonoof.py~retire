...
def n1(o):return o if not o else(lambda mi,mj,ma,mj2:[(A,(B-mi,C-mj))for(A,(B,C))in o])(*bbox(o))
...
def p(I):
	...
	for H in G:
		J=H(F);E=n1(J);A=[(A,(B,C))for(A,(B,C))in E if A in{2,4}]
		if not A:continue
		K,L=min(B for(A,(B,A))in A),min(B for(A,(A,B))in A)
		for(M,N)in o(I,A):D.append(shift(E,(M-K,N-L)))
